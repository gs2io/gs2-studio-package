...
{
    /// <summary>
    /// MonoBehaviour handler for EquipmentEnchantSlotChance. Holds one EquipmentEnchantSlotChanceBinder,
    ...
    /// EquipmentEnchantSlotChanceListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentEnchantSlotChance/EquipmentEnchantSlotChance Handler")]
    public sealed class EquipmentEnchantSlotChanceHandler : EquipmentEnchantSlotChanceHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultEquipmentEnchantSlotChanceBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IEquipmentEnchantSlotChanceBinderFactory? _binderFactory;

        private IEquipmentEnchantSlotChanceBinder? _binder;
        ...
        public override IActionableEquipmentEnchantSlotChanceBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EquipmentEnchantSlotChance"/>, so it is the model.</summary>
        public override EquipmentEnchantSlotChance? Model => _binder;
        ...
        /// to substitute a fake EquipmentEnchantSlotChanceBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IEquipmentEnchantSlotChanceBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IEquipmentEnchantSlotChanceBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultEquipmentEnchantSlotChanceBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a EquipmentEnchantSlotChanceBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentEnchantSlotChanceId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentEnchantSlotChanceBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentEnchantSlotChanceBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentEnchantSlotChanceBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentEnchantSlotChanceBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentEnchantSlotChanceBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentEnchantSlotChanceBinder binder)
        {
