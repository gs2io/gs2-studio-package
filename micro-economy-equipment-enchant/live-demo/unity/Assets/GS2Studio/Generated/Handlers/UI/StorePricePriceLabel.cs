...
{
    /// <summary>
    /// UI label bound to <c>StorePrice.price</c>.
    ...
    /// <c>StorePriceListItemHandler</c>; the
    /// handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is
    /// supplied.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/StorePrice/Label/PriceLabel")]
    public sealed class StorePricePriceLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(StorePricePriceLabel)} on '{name}': no StorePriceHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke(System.Convert.ToString(model.Price) ?? "");
        }
    }
}
