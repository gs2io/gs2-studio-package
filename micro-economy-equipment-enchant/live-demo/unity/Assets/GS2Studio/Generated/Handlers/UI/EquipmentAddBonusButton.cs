...
{
    /// <summary>
    /// UI button bound to <c>AddBonus</c> on the sibling
    /// <c>EquipmentHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EquipmentHandlerBase.Binder.AddBonus</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>EquipmentHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Equipment/ButtonAction/AddBonusButton")]
    public sealed class EquipmentAddBonusButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private EquipmentHandlerBase? _handler;
        ...
        /// <summary>
        /// Loads the arguments of <c>AddBonus</c> are read from. A row
        ...
        /// in that window is dropped, not queued: a second <c>AddBonus</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<EquipmentHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentAddBonusButton)} on '{name}': no EquipmentHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.AddBonus();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"EquipmentAddBonusButton: AddBonus failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentAddBonusButton: AddBonus failed: {ex}");
                return;
            }
            finally
            {
        ...
        private void WarnUnloadedLoadersOnce(IActionableEquipmentBinder binder)
        {
            if (_warnedUnloadedLoaders) return;
            if ((binder.LoadedUserdataInventoryEquipmentItemModel || binder.LoadedUserdataEnchantEquipmentEnchantRarityParameterModel)) return;
            _warnedUnloadedLoaders = true;
            Debug.LogWarning(
                $"{nameof(EquipmentAddBonusButton)} on '{name}': this row's mount surface did not run UserdataInventoryEquipmentItemModel, UserdataEnchantEquipmentEnchantRarityParameterModel, which fills what this component reads (PropertyId, Id); those readings render as their default, not as an absent value.", this);
        }
    }
}
