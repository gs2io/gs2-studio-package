...
namespace GS2Studio.Generated.StorePrice.UI
{
    /// <summary>
    /// UI value bound to <c>StorePrice.price</c>.
    /// Subscribes to the sibling <c>StorePriceHandler.Updated</c> event
    /// and publishes the resolved <c>double</c> value through
    ...
    /// code to any <c>UnityEvent&lt;double&gt;</c> listener. Add this
    /// component alongside (or under) a <c>StorePriceHandler</c> or
    /// <c>StorePriceListItemHandler</c>; the handler is resolved
    ...
    /// persists it.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/StorePrice/Value/PriceValue")]
    public sealed class StorePricePriceValue : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private StorePriceHandlerBase? _handler;
        [SerializeField] private UnityEvent<double> _onUpdate = new UnityEvent<double>();
        [SerializeField] private UnityEvent<double> _onSelect = new UnityEvent<double>();

        public UnityEvent<double> OnUpdate => _onUpdate;
        public UnityEvent<double> OnSelect => _onSelect;
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(StorePricePriceValue)} on '{name}': no StorePriceHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<StorePriceHandlerBase>();
        }

        private void OnUpdated(StorePrice model)
        {
            _onUpdate.Invoke(model.Price);
        }
