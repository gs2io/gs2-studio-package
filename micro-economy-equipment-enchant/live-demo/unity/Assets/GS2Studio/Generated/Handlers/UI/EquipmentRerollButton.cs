...
{
    /// <summary>
    /// UI button bound to <c>Reroll</c> on the sibling
    /// <c>EquipmentHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EquipmentHandlerBase.Binder.Reroll</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Equipment/ButtonAction/RerollButton")]
    public sealed class EquipmentRerollButton : MonoBehaviour
    {
        ...
        /// <summary>
        /// Loads the arguments of <c>Reroll</c> are read from. A row
        ...
        /// in that window is dropped, not queued: a second <c>Reroll</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentRerollButton)} on '{name}': no EquipmentHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentRerollButton: Reroll failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentRerollButton: Reroll failed: {ex}");
                return;
            }
            finally
            {
        ...
        private void WarnUnloadedLoadersOnce(IActionableEquipmentBinder binder)
        {
            ...
            Debug.LogWarning(
                $"{nameof(EquipmentRerollButton)} on '{name}': this row's mount surface did not run UserdataInventoryEquipmentItemModel, UserdataEnchantEquipmentEnchantRarityParameterModel, which fills what this component reads (PropertyId, Id); those readings render as their default, not as an absent value.", this);
        }
    }
}
