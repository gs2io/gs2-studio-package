...
{
    /// <summary>
    /// MonoBehaviour handler for Equipment. Holds one EquipmentBinder,
    ...
    /// EquipmentListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Equipment/Equipment Handler")]
    public sealed class EquipmentHandler : EquipmentHandlerBase
    {
        ...
        private IEquipmentBinder? _binder;
        ...
        /// binder implements <see cref="Equipment"/>, so it is the model.</summary>
        public override Equipment? Model => _binder;
        ...
        /// to substitute a fake EquipmentBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string propertyId, string enchant)
        {
        ...
        /// <summary>
        /// Creates a EquipmentBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentId(_id ?? string.Empty), _propertyId, new EquipmentEnchantId(_enchant ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentBinder binder)
        {
