...
{
    /// <summary>
    /// MonoBehaviour handler for EquipmentEnchantOption. Holds one EquipmentEnchantOptionBinder,
    ...
    /// EquipmentEnchantOptionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentEnchantOption/EquipmentEnchantOption Handler")]
    public sealed class EquipmentEnchantOptionHandler : EquipmentEnchantOptionHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultEquipmentEnchantOptionBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IEquipmentEnchantOptionBinderFactory? _binderFactory;

        private IEquipmentEnchantOptionBinder? _binder;
        ...
        public override IActionableEquipmentEnchantOptionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EquipmentEnchantOption"/>, so it is the model.</summary>
        public override EquipmentEnchantOption? Model => _binder;
        ...
        /// to substitute a fake EquipmentEnchantOptionBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IEquipmentEnchantOptionBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IEquipmentEnchantOptionBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultEquipmentEnchantOptionBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a EquipmentEnchantOptionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentEnchantOptionId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentEnchantOptionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentEnchantOptionBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentEnchantOptionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentEnchantOptionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentEnchantOptionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentEnchantOptionBinder binder)
        {
