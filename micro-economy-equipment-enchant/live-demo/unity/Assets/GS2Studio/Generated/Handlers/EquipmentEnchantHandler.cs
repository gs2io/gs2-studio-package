...
{
    /// <summary>
    /// MonoBehaviour handler for EquipmentEnchant. Holds one EquipmentEnchantBinder,
    ...
    /// EquipmentEnchantListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentEnchant/EquipmentEnchant Handler")]
    public sealed class EquipmentEnchantHandler : EquipmentEnchantHandlerBase
    {
        ...
        private IEquipmentEnchantBinder? _binder;
        ...
        public override IActionableEquipmentEnchantBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EquipmentEnchant"/>, so it is the model.</summary>
        public override EquipmentEnchant? Model => _binder;
        ...
        /// to substitute a fake EquipmentEnchantBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a EquipmentEnchantBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentEnchantId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentEnchantBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentEnchantBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentEnchantBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentEnchantBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentEnchantBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentEnchantBinder binder)
        {
