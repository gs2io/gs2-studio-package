...
{
    /// <summary>
    /// Public read contract for the EquipmentCatalog binder collection: the
    ...
    /// </summary>
    public interface IReadOnlyEquipmentCatalogBinderCollection : IReadOnlyList<IActionableEquipmentCatalogBinder>
    {
        IReadOnlyEquipmentCatalogBinderCollection WhereEquipment(EquipmentId equipment);
    }

    /// <summary>
    /// Owning contract for the EquipmentCatalog binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlyEquipmentCatalogBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface IEquipmentCatalogBinderCollection : IReadOnlyEquipmentCatalogBinderCollection, IDisposable
    {
        IComparer<IReadOnlyEquipmentCatalogBinder> Comparer { get; set; }
        event Action<IEquipmentCatalogBinder> ItemAdded;
        event Action<IEquipmentCatalogBinder> ItemRemoved;
        void Invalidate();
        Task MountFromExchangeEquipmentTakeMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromExchangeEquipmentTakeMasterData(Action? onChange = null, Action<Exception>? onError = null);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="EquipmentCatalogBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableEquipmentCatalogBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlyEquipmentCatalogBinderCollection : IReadOnlyEquipmentCatalogBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly EquipmentCatalogBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlyEquipmentCatalogBinder>? _predicate;
        private readonly List<IActionableEquipmentCatalogBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableEquipmentCatalogBinder> EmptyList = new List<IActionableEquipmentCatalogBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlyEquipmentCatalogBinderCollection Empty { get; } = new ReadOnlyEquipmentCatalogBinderCollection();
        ...
        /// </summary>
        protected ReadOnlyEquipmentCatalogBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlyEquipmentCatalogBinderCollection(
            EquipmentCatalogBinderCollection rootSource,
            Predicate<IReadOnlyEquipmentCatalogBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableEquipmentCatalogBinder>();
        ...
        public virtual IActionableEquipmentCatalogBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableEquipmentCatalogBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual EquipmentCatalogBinderCollection? GetRootSource() => _rootSource;
        ...
        public IReadOnlyEquipmentCatalogBinderCollection WhereEquipment(EquipmentId equipment)
        {
            ThrowIfDisposedForNavigation();
            if (_dead) return Empty;
            if (EqualityComparer<EquipmentId>.Default.Equals(equipment, default)) return Empty;
            var root = GetRootSource();
            if (root == null) return Empty;
            var existing = _predicate;
            Predicate<IReadOnlyEquipmentCatalogBinder> composed = existing == null
                ? (Predicate<IReadOnlyEquipmentCatalogBinder>)(b => MatchEquipment(b, equipment))
                : b => existing(b) && MatchEquipment(b, equipment);
            return new ReadOnlyEquipmentCatalogBinderCollection(root, composed);
        }
        private static bool MatchEquipment(IReadOnlyEquipmentCatalogBinder b, EquipmentId equipment)
            => EqualityComparer<EquipmentId>.Default.Equals(b.Equipment, equipment);
        #endregion

    }

    /// <summary>
    /// Collection binder for EquipmentCatalog.
    /// Wraps a list of EquipmentCatalogBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class EquipmentCatalogBinderCollection : ReadOnlyEquipmentCatalogBinderCollection, IEquipmentCatalogBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;

        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableEquipmentCatalogBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning EquipmentCatalogBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<EquipmentCatalogBinder> _binders = new List<EquipmentCatalogBinder>();
        ...
        private readonly Dictionary<string, EquipmentCatalogBinder> _bindersByRowKey = new Dictionary<string, EquipmentCatalogBinder>();
        ...
        private readonly List<WeakReference<ReadOnlyEquipmentCatalogBinderCollection>> _derivedViews = new List<WeakReference<ReadOnlyEquipmentCatalogBinderCollection>>();
        ...
        private IComparer<IReadOnlyEquipmentCatalogBinder> _configuredComparer = EquipmentCatalogBinderComparer.Default;
        private IComparer<IReadOnlyEquipmentCatalogBinder> _effectiveComparer =
            new EquipmentCatalogBinderIdTieBreakComparer(EquipmentCatalogBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlyEquipmentCatalogBinder so
        ...
        public IComparer<IReadOnlyEquipmentCatalogBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new EquipmentCatalogBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableEquipmentCatalogBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableEquipmentCatalogBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableEquipmentCatalogBinder contract.
        /// </summary>
        public override IEnumerator<IActionableEquipmentCatalogBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override EquipmentCatalogBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (IEquipmentCatalogBinder).
        // Public so a fake IEquipmentCatalogBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableEquipmentCatalogBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<IEquipmentCatalogBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<IEquipmentCatalogBinder>? ItemRemoved;

        public EquipmentCatalogBinderCollection(
            Gs2Domain gs2,
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlyEquipmentCatalogBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlyEquipmentCatalogBinderCollection>(view));
        }
        ...
        /// <summary>One-shot Create + MountFromExchangeEquipmentTakeMasterDataAsync (no subscription).</summary>
        public static async Task<EquipmentCatalogBinderCollection> CreateFromExchangeEquipmentTakeMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new EquipmentCatalogBinderCollection(gs2, session);
            await coll.MountFromExchangeEquipmentTakeMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromExchangeEquipmentTakeMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
        ...
        public void SubscribeFromExchangeEquipmentTakeMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromExchangeEquipmentTakeMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromExchangeEquipmentTakeMasterItems(IList<Gs2.Unity.Gs2Exchange.Model.EzRateModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            ...
            {
                if (_disposed) return;
                var rowKey = ExtractExchangeEquipmentTakeMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyExchangeEquipmentTakeMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromExchangeEquipmentTakeMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    {
                        ApplyExchangeEquipmentTakeMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
        ...
        private async Task<EquipmentCatalogBinder> BuildBinderFromExchangeEquipmentTakeMasterItem(Gs2.Unity.Gs2Exchange.Model.EzRateModel item, CancellationToken cancellationToken)
        {
            ...
            var binder = new EquipmentCatalogBinder(model, _gs2, _session);
        ...
        internal sealed class EquipmentCatalogBinderComparer : IComparer<IReadOnlyEquipmentCatalogBinder>
        {
            public static readonly EquipmentCatalogBinderComparer Default = new EquipmentCatalogBinderComparer();
            public int Compare(IReadOnlyEquipmentCatalogBinder? a, IReadOnlyEquipmentCatalogBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<EquipmentCatalogId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class EquipmentCatalogBinderIdTieBreakComparer : IComparer<IReadOnlyEquipmentCatalogBinder>
        {
            private readonly IComparer<IReadOnlyEquipmentCatalogBinder> _inner;
            public EquipmentCatalogBinderIdTieBreakComparer(IComparer<IReadOnlyEquipmentCatalogBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlyEquipmentCatalogBinder? a, IReadOnlyEquipmentCatalogBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
