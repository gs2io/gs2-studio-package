...
namespace GS2Studio.Generated.Character
{
    ...
    public interface ICharacterBinderCollection : IReadOnlyCharacterBinderCollection, IDisposable
    {
        ...
        Task MountFromExchangeCharacterLimitBreakMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromExchangeCharacterLimitBreakMasterData(Action? onChange = null, Action<Exception>? onError = null);
        Task MountFromExchangeCharacterGradeTrainMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromExchangeCharacterGradeTrainMasterData(Action? onChange = null, Action<Exception>? onError = null);
    ...
    public sealed class CharacterBinderCollection : ReadOnlyCharacterBinderCollection, ICharacterBinderCollection
    {
        ...
        public void Invalidate()
        {
            ...
            new Gs2Bind.Gs2Exchange.RateModelArrayLoader("CharacterLimitBreak").Invalidate(_gs2, _session);
            new Gs2Bind.Gs2Exchange.RateModelArrayLoader("CharacterGradeTrain").Invalidate(_gs2, _session);
        ...
        /// <summary>One-shot Create + MountFromExchangeCharacterLimitBreakMasterDataAsync (no subscription).</summary>
        public static async Task<CharacterBinderCollection> CreateFromExchangeCharacterLimitBreakMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new CharacterBinderCollection(gs2, session);
            await coll.MountFromExchangeCharacterLimitBreakMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromExchangeCharacterLimitBreakMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
            var arrayLoader = new Gs2Bind.Gs2Exchange.RateModelArrayLoader("CharacterLimitBreak");
            var items = await arrayLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            await ReconcileFromExchangeCharacterLimitBreakMasterItems(items, attachChildSubscribe: false, cancellationToken);
            _mounted = true;
        }

        public void SubscribeFromExchangeCharacterLimitBreakMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            if (_subscriptionActive) throw new InvalidOperationException("Already subscribed");
            _subscriptionActive = true;
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromExchangeCharacterLimitBreakMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
                        if (onError != null) onError(ex); else UnityEngine.Debug.LogException(ex);
        ...
        private async Task ReconcileFromExchangeCharacterLimitBreakMasterItems(IList<Gs2.Unity.Gs2Exchange.Model.EzRateModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            var seen = new HashSet<string>();
            foreach (var item in items)
            {
                if (_disposed) return;
                var rowKey = ExtractExchangeCharacterLimitBreakMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyExchangeCharacterLimitBreakMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromExchangeCharacterLimitBreakMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    if (_bindersByRowKey.TryGetValue(rowKey, out var raced))
                    {
                        ApplyExchangeCharacterLimitBreakMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
                    if (attachChildSubscribe) binder.Subscribe(_onChange);
        ...
        private async Task<CharacterBinder> BuildBinderFromExchangeCharacterLimitBreakMasterItem(Gs2.Unity.Gs2Exchange.Model.EzRateModel item, CancellationToken cancellationToken)
        {
            var model = CharacterBinder.CreateModel((string.IsNullOrEmpty(item.Name) ? default(CharacterId) : new CharacterId(item.Name)), string.Empty);
            ApplyExchangeCharacterLimitBreakMasterItemTo(model, item);
            var binder = new CharacterBinder(model, _gs2, _session, _mountSurface: CharacterMountSurface.ExchangeCharacterLimitBreakMaster);
            await binder.MountAsync(cancellationToken);
            return binder;
        }

        private static void ApplyExchangeCharacterLimitBreakMasterItemTo(MutableCharacter model, Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            // This loader carries no master-item field assignments; reconcile manages
            // membership only (per-element field changes are tracked by each element
            // binder's own Subscribe).
        ...
        private CharacterId ExtractExchangeCharacterLimitBreakMasterIdentity(Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            return (string.IsNullOrEmpty(item.Name) ? default(CharacterId) : new CharacterId(item.Name));
        }

        private string? ExtractExchangeCharacterLimitBreakMasterRowKey(Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            var id = ExtractExchangeCharacterLimitBreakMasterIdentity(item);
            if (EqualityComparer<CharacterId>.Default.Equals(id, default)) return null;
            if (string.IsNullOrEmpty(item.Name)) return null;
            return $"{item.Name}";
        }
        /// <summary>One-shot Create + MountFromExchangeCharacterGradeTrainMasterDataAsync (no subscription).</summary>
        public static async Task<CharacterBinderCollection> CreateFromExchangeCharacterGradeTrainMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new CharacterBinderCollection(gs2, session);
            await coll.MountFromExchangeCharacterGradeTrainMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromExchangeCharacterGradeTrainMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
            var arrayLoader = new Gs2Bind.Gs2Exchange.RateModelArrayLoader("CharacterGradeTrain");
            var items = await arrayLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            await ReconcileFromExchangeCharacterGradeTrainMasterItems(items, attachChildSubscribe: false, cancellationToken);
            _mounted = true;
        }

        public void SubscribeFromExchangeCharacterGradeTrainMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            if (_subscriptionActive) throw new InvalidOperationException("Already subscribed");
            _subscriptionActive = true;
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromExchangeCharacterGradeTrainMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
                        if (onError != null) onError(ex); else UnityEngine.Debug.LogException(ex);
        ...
        private async Task ReconcileFromExchangeCharacterGradeTrainMasterItems(IList<Gs2.Unity.Gs2Exchange.Model.EzRateModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            var seen = new HashSet<string>();
            foreach (var item in items)
            {
                if (_disposed) return;
                var rowKey = ExtractExchangeCharacterGradeTrainMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyExchangeCharacterGradeTrainMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromExchangeCharacterGradeTrainMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    if (_bindersByRowKey.TryGetValue(rowKey, out var raced))
                    {
                        ApplyExchangeCharacterGradeTrainMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
                    if (attachChildSubscribe) binder.Subscribe(_onChange);
        ...
        private async Task<CharacterBinder> BuildBinderFromExchangeCharacterGradeTrainMasterItem(Gs2.Unity.Gs2Exchange.Model.EzRateModel item, CancellationToken cancellationToken)
        {
            var model = CharacterBinder.CreateModel((string.IsNullOrEmpty(item.Name) ? default(CharacterId) : new CharacterId(item.Name)), string.Empty);
            ApplyExchangeCharacterGradeTrainMasterItemTo(model, item);
            var binder = new CharacterBinder(model, _gs2, _session, _mountSurface: CharacterMountSurface.ExchangeCharacterGradeTrainMaster);
            await binder.MountAsync(cancellationToken);
            return binder;
        }

        private static void ApplyExchangeCharacterGradeTrainMasterItemTo(MutableCharacter model, Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            // This loader carries no master-item field assignments; reconcile manages
            // membership only (per-element field changes are tracked by each element
            // binder's own Subscribe).
        ...
        private CharacterId ExtractExchangeCharacterGradeTrainMasterIdentity(Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            return (string.IsNullOrEmpty(item.Name) ? default(CharacterId) : new CharacterId(item.Name));
        }

        private string? ExtractExchangeCharacterGradeTrainMasterRowKey(Gs2.Unity.Gs2Exchange.Model.EzRateModel item)
        {
            var id = ExtractExchangeCharacterGradeTrainMasterIdentity(item);
            if (EqualityComparer<CharacterId>.Default.Equals(id, default)) return null;
            if (string.IsNullOrEmpty(item.Name)) return null;
            return $"{item.Name}";
        }
