...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterGradeStep. Holds one CharacterGradeStepBinder,
    ...
    /// CharacterGradeStepListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterGradeStep/CharacterGradeStep Handler")]
    public sealed class CharacterGradeStepHandler : CharacterGradeStepHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterGradeStepBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterGradeStepBinderFactory? _binderFactory;

        private ICharacterGradeStepBinder? _binder;
        ...
        public override IActionableCharacterGradeStepBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterGradeStep"/>, so it is the model.</summary>
        public override CharacterGradeStep? Model => _binder;
        ...
        /// to substitute a fake CharacterGradeStepBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterGradeStepBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterGradeStepBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterGradeStepBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CharacterGradeStepBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterGradeStepId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterGradeStepBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterGradeStepBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterGradeStepBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterGradeStepBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterGradeStepBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterGradeStepBinder binder)
        {
