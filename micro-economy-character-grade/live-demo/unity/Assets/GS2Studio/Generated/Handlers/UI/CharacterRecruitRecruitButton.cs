...
using GS2Studio.Generated.CharacterRecruit;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.CharacterRecruit.UI
{
    /// <summary>
    /// UI button bound to <c>Recruit</c> on the sibling
    /// <c>CharacterRecruitHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterRecruitHandlerBase.Binder.Recruit</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>CharacterRecruitHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterRecruit/ButtonAction/RecruitButton")]
    public sealed class CharacterRecruitRecruitButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private CharacterRecruitHandlerBase? _handler;
        ...
        /// in that window is dropped, not queued: a second <c>Recruit</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<CharacterRecruitHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterRecruitRecruitButton)} on '{name}': no CharacterRecruitHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"CharacterRecruitRecruitButton: Recruit failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterRecruitRecruitButton: Recruit failed: {ex}");
                return;
            }
            finally
            {
