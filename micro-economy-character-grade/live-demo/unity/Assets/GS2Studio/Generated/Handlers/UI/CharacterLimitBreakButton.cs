...
{
    /// <summary>
    /// UI button bound to <c>LimitBreak</c> on the sibling
    /// <c>CharacterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterHandlerBase.Binder.LimitBreak</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/ButtonAction/LimitBreakButton")]
    public sealed class CharacterLimitBreakButton : MonoBehaviour
    {
        ...
        /// <summary>
        /// Loads the arguments of <c>LimitBreak</c> are read from. A row
        ...
        /// in that window is dropped, not queued: a second <c>LimitBreak</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterLimitBreakButton)} on '{name}': no CharacterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.LimitBreak();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"CharacterLimitBreakButton: LimitBreak failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterLimitBreakButton: LimitBreak failed: {ex}");
                return;
            }
            finally
            {
        ...
        private void WarnUnloadedLoadersOnce(IActionableCharacterBinder binder)
        {
            ...
            Debug.LogWarning(
                $"{nameof(CharacterLimitBreakButton)} on '{name}': this row's mount surface did not run UserdataInventoryCharacterItemModel, which fills what this component reads (PropertyId, Id); those readings render as their default, not as an absent value.", this);
        }
    }
}
