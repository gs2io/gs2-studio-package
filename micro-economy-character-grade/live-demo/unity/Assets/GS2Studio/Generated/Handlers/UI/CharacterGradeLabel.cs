...
namespace GS2Studio.Generated.Character.UI
{
    ...
    /// <c>Grade {grade}</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/TemplateLabel/GradeLabel")]
    public sealed class CharacterGradeLabel : MonoBehaviour
    {
        ...
        /// </summary>
        public static readonly string[] RequiredLoaders = { "UserdataGradeCharacterGradeGradeModel" };
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterGradeLabel)} on '{name}': no CharacterHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void OnUpdated(Character model)
        {
            ...
            _onUpdate.Invoke($"Grade {model.Grade}");
        }
        ...
        private void WarnUnloadedLoadersOnce()
        {
            ...
            if (binder.LoadedUserdataGradeCharacterGradeGradeModel) return;
            _warnedUnloadedLoaders = true;
            Debug.LogWarning(
                $"{nameof(CharacterGradeLabel)} on '{name}': this row's mount surface did not run UserdataGradeCharacterGradeGradeModel, which fills what this component reads (Grade); those readings render as their default, not as an absent value.", this);
        }
    }
}
