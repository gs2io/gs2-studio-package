...
{
    /// <summary>
    /// UI button bound to <c>TrainHard</c> on the sibling
    /// <c>CharacterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterHandlerBase.Binder.TrainHard</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/ButtonAction/TrainHardButton")]
    public sealed class CharacterTrainHardButton : MonoBehaviour
    {
        ...
        /// <summary>
        /// Loads the arguments of <c>TrainHard</c> are read from. A row
        ...
        /// in that window is dropped, not queued: a second <c>TrainHard</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterTrainHardButton)} on '{name}': no CharacterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.TrainHard();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"CharacterTrainHardButton: TrainHard failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterTrainHardButton: TrainHard failed: {ex}");
                return;
            }
            finally
            {
        ...
        private void WarnUnloadedLoadersOnce(IActionableCharacterBinder binder)
        {
            ...
            Debug.LogWarning(
                $"{nameof(CharacterTrainHardButton)} on '{name}': this row's mount surface did not run UserdataInventoryCharacterItemModel, which fills what this component reads (PropertyId, Id); those readings render as their default, not as an absent value.", this);
        }
    }
}
