...
{
    /// <summary>
    /// UI label bound to <c>PaidDeposit.count</c>.
    /// Subscribes to the sibling <c>PaidDepositHandler.Updated</c> event
    ...
    /// component alongside (or under) a <c>PaidDepositHandler</c> or
    /// <c>PaidDepositListItemHandler</c>; the
    /// handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is
    /// supplied.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/PaidDeposit/Label/CountLabel")]
    public sealed class PaidDepositCountLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(PaidDepositCountLabel)} on '{name}': no PaidDepositHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
