...
{
    /// <summary>
    /// UI label bound to <c>Wallet.free</c>.
    ...
    /// supplied.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Wallet/Label/FreeBalanceLabel")]
    public sealed class WalletFreeBalanceLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(WalletFreeBalanceLabel)} on '{name}': no WalletHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke(System.Convert.ToString(model.Free) ?? "");
        }
    }
}
