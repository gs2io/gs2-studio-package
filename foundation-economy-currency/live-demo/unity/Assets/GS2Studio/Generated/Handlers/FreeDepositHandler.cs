...
{
    /// <summary>
    /// MonoBehaviour handler for FreeDeposit. Holds one FreeDepositBinder,
    ...
    /// FreeDepositListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/FreeDeposit/FreeDeposit Handler")]
    public sealed class FreeDepositHandler : FreeDepositHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultFreeDepositBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IFreeDepositBinderFactory? _binderFactory;

        private IFreeDepositBinder? _binder;
        ...
        public override IActionableFreeDepositBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="FreeDeposit"/>, so it is the model.</summary>
        public override FreeDeposit? Model => _binder;
        ...
        /// to substitute a fake FreeDepositBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IFreeDepositBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IFreeDepositBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultFreeDepositBinderFactory.Instance;
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new FreeDepositId("freedeposit"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned FreeDepositBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a FreeDepositBinderCollection) already
        ...
        /// </summary>
        public void Bind(IFreeDepositBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IFreeDepositBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IFreeDepositBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IFreeDepositBinder binder)
        {
