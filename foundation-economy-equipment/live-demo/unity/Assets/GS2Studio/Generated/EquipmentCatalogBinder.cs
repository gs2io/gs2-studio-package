...
{
    /// <summary>
    /// Pure read contract for <see cref="EquipmentCatalogBinder"/>: the read-only
    /// model surface (<see cref="EquipmentCatalog"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyEquipmentCatalogBinder : EquipmentCatalog
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="EquipmentCatalogBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableEquipmentCatalogBinder : IReadOnlyEquipmentCatalogBinder
    {
        Task Take(Gs2.Unity.Gs2Exchange.Model.EzConfig[]? config = null);
    }

    /// <summary>
    /// Owning contract for <see cref="EquipmentCatalogBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableEquipmentCatalogBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface IEquipmentCatalogBinder : IActionableEquipmentCatalogBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="EquipmentCatalogBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="EquipmentCatalog"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyEquipmentCatalogBinder : IReadOnlyEquipmentCatalogBinder
    {
        ...
        private protected readonly MutableEquipmentCatalog _model;
        ...
        /// the read-only <c>EquipmentCatalog</c> surface this binder implements.
        /// </summary>
        internal MutableEquipmentCatalog MutableModel => _model;
        /// <inheritdoc cref="EquipmentCatalog.Id" />
        public EquipmentCatalogId Id => _model.Id;
        /// <inheritdoc cref="EquipmentCatalog.Equipment" />
        public EquipmentId Equipment => _model.Equipment;
        /// <summary>
        ...
        /// </summary>
        private protected ReadOnlyEquipmentCatalogBinder(MutableEquipmentCatalog model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyEquipmentCatalogBinder"/>) and
    ...
    /// <summary>
    /// Binder for EquipmentCatalog
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class EquipmentCatalogBinder : ReadOnlyEquipmentCatalogBinder, IEquipmentCatalogBinder
    {
        ...
        /// </summary>
        internal EquipmentCatalogBinder(
            MutableEquipmentCatalog model,
            Gs2Domain gs2,
        ...
        /// <summary>
        /// Creates a EquipmentCatalog instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableEquipmentCatalog CreateModel(EquipmentCatalogId id)
        {
            var model = new MutableEquipmentCatalog();
            model.Id = id;
            return model;
        }

        /// <summary>
        /// Creates a EquipmentCatalogBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<EquipmentCatalogBinder> CreateAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var binder = new EquipmentCatalogBinder(model, gs2, session);
        ...
        public async Task MountAsync(CancellationToken cancellationToken = default)
        {
            ...
            var _exchangeEquipmentTakeNamespaceRateModel = await __exchangeEquipmentTakeNamespaceRateModelLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            if (_exchangeEquipmentTakeNamespaceRateModel != null)
            {
            }
            ...
            _model.Equipment = new EquipmentId(string.Empty);
            if (_transactionAcquireAction != null) RestoreEquipment(_model, _transactionAcquireAction.Action, _transactionAcquireAction.Request);
            _mounted = true;
        }
        ...
        {
            ThrowIfDisposed();
            _unsubscribers.Add(__exchangeEquipmentTakeNamespaceRateModelLoader.Subscribe(
                _gs2,
                _session,
                (_, _, value) =>
                {
            ...
            _unsubscribers.Add(__transactionAcquireActionLoader.Subscribe(
                ...
                (_, _, value) =>
                {
                    ...
                    _model.Equipment = new EquipmentId(string.Empty);
                    if (value != null) RestoreEquipment(_model, value.Action, value.Request);
                    return Task.CompletedTask;
                },
                () => onChange?.Invoke()
            ));
        ...
        {
            ThrowIfDisposed();
            __exchangeEquipmentTakeNamespaceRateModelLoader.Invalidate(_gs2, _session);
        ...
        /// (callable through <see cref="IReadOnlyEquipmentCatalogBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(EquipmentCatalogBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
        ...
        public async Task Take(Gs2.Unity.Gs2Exchange.Model.EzConfig[]? config = null)
        {
            EnsureActionContext();
            await new Gs2Bind.Gs2Exchange.RateModelLoader("EquipmentTake", _model.Id).Exchange(_gs2, _session, count: 1, config: config);
        }
        #endregion

        #region MasterData reverse decode
        public static bool RestoreEquipment(IMutableEquipmentCatalog model, string actionName, string requestJson)
        {
            if (requestJson == null) return false;
            var request = JsonMapper.ToObject(requestJson);
            if (actionName == "Gs2Inventory:AcquireItemSetByUserId" && ReadRequestValue(request, new string[] { "namespaceName" }) == "Equipment" && ReadRequestValue(request, new string[] { "inventoryName" }) == "Equipment" && ReadRequestValue(request, new string[] { "acquireCount" }) == (1L).ToString())
            {
                var __value = ReadRequestValue(request, new string[] { "itemName" });
                if (__value != null)
                {
                    model.Equipment = (EquipmentId)__value;
                    return true;
                }
            }
            return false;
