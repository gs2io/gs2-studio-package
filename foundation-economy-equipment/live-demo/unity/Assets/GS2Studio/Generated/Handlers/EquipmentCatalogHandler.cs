...
{
    /// <summary>
    /// MonoBehaviour handler for EquipmentCatalog. Holds one EquipmentCatalogBinder,
    ...
    /// EquipmentCatalogListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentCatalog/EquipmentCatalog Handler")]
    public sealed class EquipmentCatalogHandler : EquipmentCatalogHandlerBase
    {
        ...
        private IEquipmentCatalogBinder? _binder;
        ...
        public override IActionableEquipmentCatalogBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EquipmentCatalog"/>, so it is the model.</summary>
        public override EquipmentCatalog? Model => _binder;
        ...
        /// to substitute a fake EquipmentCatalogBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a EquipmentCatalogBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentCatalogId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentCatalogBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentCatalogBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentCatalogBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentCatalogBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentCatalogBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentCatalogBinder binder)
        {
