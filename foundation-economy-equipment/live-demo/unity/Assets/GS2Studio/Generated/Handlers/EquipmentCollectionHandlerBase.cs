...
namespace GS2Studio.Generated.EquipmentCollection
{
    ...
    /// EquipmentCollectionBinder: the standalone/list EquipmentCollectionHandler and the
    /// slot-bound EquipmentCollectionListItemHandler. Concrete handlers supply the
    ...
    /// </summary>
    public abstract class EquipmentCollectionHandlerBase : MonoBehaviour
    {
        /// <summary>The bound binder (non-owning actionable view), or null when none is attached.</summary>
        public abstract IActionableEquipmentCollectionBinder? Binder { get; }

        /// <summary>The bound model, or null when no binder is attached.</summary>
        public abstract EquipmentCollection? Model { get; }
        ...
        public event Action<EquipmentCollection>? Updated;

        /// <summary>Fires once each time a binder is attached (non-owning actionable view).</summary>
        public event Action<IActionableEquipmentCollectionBinder>? Bound;
        ...
        protected void RaiseUpdated(EquipmentCollection model) => Updated?.Invoke(model);
        protected void RaiseBound(IActionableEquipmentCollectionBinder binder) => Bound?.Invoke(binder);
