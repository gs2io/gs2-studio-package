...
{
    /// <summary>
    /// MonoBehaviour handler for EquipmentCollection. Holds one EquipmentCollectionBinder,
    ...
    /// EquipmentCollectionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentCollection/EquipmentCollection Handler")]
    public sealed class EquipmentCollectionHandler : EquipmentCollectionHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultEquipmentCollectionBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IEquipmentCollectionBinderFactory? _binderFactory;

        private IEquipmentCollectionBinder? _binder;
        ...
        public override IActionableEquipmentCollectionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EquipmentCollection"/>, so it is the model.</summary>
        public override EquipmentCollection? Model => _binder;
        ...
        /// to substitute a fake EquipmentCollectionBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IEquipmentCollectionBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IEquipmentCollectionBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultEquipmentCollectionBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a EquipmentCollectionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EquipmentCollectionId("equipmentcollection"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EquipmentCollectionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EquipmentCollectionBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEquipmentCollectionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEquipmentCollectionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEquipmentCollectionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEquipmentCollectionBinder binder)
        {
