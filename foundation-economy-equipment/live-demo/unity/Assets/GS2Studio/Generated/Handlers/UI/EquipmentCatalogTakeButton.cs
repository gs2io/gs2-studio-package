...
using GS2Studio.Generated.EquipmentCatalog;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.EquipmentCatalog.UI
{
    /// <summary>
    /// UI button bound to <c>Take</c> on the sibling
    /// <c>EquipmentCatalogHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EquipmentCatalogHandlerBase.Binder.Take</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>EquipmentCatalogHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentCatalog/ButtonAction/TakeButton")]
    public sealed class EquipmentCatalogTakeButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private EquipmentCatalogHandlerBase? _handler;
        ...
        /// in that window is dropped, not queued: a second <c>Take</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<EquipmentCatalogHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentCatalogTakeButton)} on '{name}': no EquipmentCatalogHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentCatalogTakeButton: Take failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentCatalogTakeButton: Take failed: {ex}");
                return;
            }
            finally
            {
