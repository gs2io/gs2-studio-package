...
namespace GS2Studio.Generated.EquipmentCatalog.UI
{
    ...
    /// <c>{id}</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>EquipmentCatalog</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentCatalog/TemplateLabel/NameLabel")]
    public sealed class EquipmentCatalogNameLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentCatalogNameLabel)} on '{name}': no EquipmentCatalogHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void OnUpdated(EquipmentCatalog model)
        {
            _onUpdate.Invoke($"{model.Id.Value}");
        }
    }
}
