...
{
    /// <summary>
    /// UI button bound to <c>Discard</c> on the sibling
    /// <c>EquipmentHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EquipmentHandlerBase.Binder.Discard</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Equipment/ButtonAction/DiscardButton")]
    public sealed class EquipmentDiscardButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Discard</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentDiscardButton)} on '{name}': no EquipmentHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentDiscardButton: Discard failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EquipmentDiscardButton: Discard failed: {ex}");
                return;
            }
            finally
            {
