...
namespace GS2Studio.Generated.Equipment.UI
{
    ...
    public sealed class EquipmentPropertyIdValue : MonoBehaviour
    {
        ...
        /// </summary>
        public static readonly string[] RequiredLoaders = { "UserdataInventoryEquipmentItemModel" };
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EquipmentPropertyIdValue)} on '{name}': no EquipmentHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void WarnUnloadedLoadersOnce()
        {
            ...
            if (binder.LoadedUserdataInventoryEquipmentItemModel) return;
            _warnedUnloadedLoaders = true;
            Debug.LogWarning(
                $"{nameof(EquipmentPropertyIdValue)} on '{name}': this row's mount surface did not run UserdataInventoryEquipmentItemModel, which fills what this component reads (PropertyId); those readings render as their default, not as an absent value.", this);
        }
