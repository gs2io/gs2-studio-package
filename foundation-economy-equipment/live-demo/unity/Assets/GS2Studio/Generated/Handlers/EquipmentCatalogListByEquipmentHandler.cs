...
{
    /// <summary>
    /// MonoBehaviour list handler for EquipmentCatalog scoped by the
    /// Equipment reference. Auto-resolves a parent
    /// EquipmentHandlerBase from the GameObject hierarchy, owns a
    /// EquipmentCatalogBinderCollection, and instantiates a child
    /// EquipmentCatalogListItemHandler only for the binders whose
    /// Equipment equals the parent's id. Implements
    /// IEquipmentCatalogBinderListSource so items read Binders[Index] through the
    /// shared parent-list surface.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EquipmentCatalog/EquipmentCatalog List By Equipment Handler")]
    public sealed class EquipmentCatalogListByEquipmentHandler : MonoBehaviour, IEquipmentCatalogBinderListSource
    {
        ...
        [SerializeField] private EquipmentHandlerBase? _parentHandler;
        ...
        private EquipmentHandlerBase? _subscribedParent;
        ...
        private readonly Dictionary<IActionableEquipmentCatalogBinder, IEquipmentCatalogBinder> _binderByView =
            new Dictionary<IActionableEquipmentCatalogBinder, IEquipmentCatalogBinder>();
        ...
        private readonly List<IActionableEquipmentCatalogBinder> _visibleBinders =
            new List<IActionableEquipmentCatalogBinder>();
        ...
        public IReadOnlyList<IActionableEquipmentCatalogBinder> Binders => _visibleBinders;
        ...
        private IEquipmentCatalogBinderFactory ResolveBinderFactory()
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            var operationGeneration = ++_reloadGeneration;
            IEquipmentCatalogBinderCollection? operationCollection = null;
            try
            {
                Cleanup();
                ...
                if (_parentHandler == null)
                    throw new InvalidOperationException("Parent EquipmentHandlerBase is not assigned and none was found in the parent hierarchy.");
                SubscribeParent();
                var parentBinder = _parentHandler.Binder;
                if (parentBinder == null)
                    throw new InvalidOperationException("Parent EquipmentHandlerBase has no bound binder yet.");
                ...
                if (_autoSubscribe)
                {
                    ...
                    collection.SubscribeFromExchangeEquipmentTakeMasterData(collectionChanged, collectionFailed);
                }
        ...
        private void ResolveParentHandler()
        {
            ...
            // EquipmentHandlerBase GameObject.
            if (_parentHandler == null)
            {
                _parentHandler = GetComponentInParent<EquipmentHandlerBase>();
            }
        }
        ...
        /// </summary>
        private void OnParentBound(IActionableEquipmentBinder binder)
        {
            _parentId = binder.Id;
            var collection = _collection;
            if (collection == null) return;
        ...
        // Keep this predicate in sync with the WhereEquipment
        // match helper emitted on EquipmentCatalogBinderCollection (see the
        ...
        private bool MatchesParent(IReadOnlyEquipmentCatalogBinder binder)
            => EqualityComparer<EquipmentId>.Default.Equals(binder.Equipment, _parentId);

        private bool IsCurrentOperation(long operationGeneration, IEquipmentCatalogBinderCollection? operationCollection)
            => !_isDestroyed
        ...
        {
            Action<IEquipmentCatalogBinder> itemAdded = binder =>
                OnItemAdded(operationGeneration, collection, binder);
            Action<IEquipmentCatalogBinder> itemRemoved = binder =>
        ...
        private void OnItemAdded(
            long operationGeneration,
            IEquipmentCatalogBinderCollection collection,
            IEquipmentCatalogBinder binder)
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
        ...
        private void OnCollectionChanged(long operationGeneration, IEquipmentCatalogBinderCollection collection)
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
        ...
        private int IndexOf(IActionableEquipmentCatalogBinder binder)
        {
        ...
        private void DestroyItem(IActionableEquipmentCatalogBinder binder)
        {
            ...
            IEquipmentCatalogBinderCollection collection)
        {
            ...
            var visibleSet = new HashSet<IActionableEquipmentCatalogBinder>(_visibleBinders);
        ...
        private void Cleanup()
        {
            ...
            var handlers = new List<EquipmentCatalogListItemHandler>(_handlers.Values);
            _collection = null;
