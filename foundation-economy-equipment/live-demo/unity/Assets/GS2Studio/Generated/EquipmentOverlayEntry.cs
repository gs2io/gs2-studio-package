// <auto-generated>
//     This code was generated by CSharpScriptableObjectGeneratorService.
//     Do not modify this file directly.
// </auto-generated>

#nullable enable
using System;
using UnityEngine;
using GS2Studio.Generated.EquipmentCategory;

namespace GS2Studio.Generated.Equipment
{
    /// <summary>
    /// AssetDelivery overlay entry for Equipment.
    /// Field values are populated from the studio2-emitted overlay payload
    /// at AssetBundle build time.
    /// </summary>
    public class EquipmentOverlayEntry : ScriptableObject
    {
        [SerializeField] private string _rawInstanceId = default!;
        [SerializeField] private string _category = default!;

        public string RawInstanceId => _rawInstanceId;

        public EquipmentCategoryId Category => new EquipmentCategoryId(_category);

        /// <summary>
        /// Copy the overlay-bound values into <paramref name="model"/>. Public
        /// against the public read-write contract so external stubs can apply
        /// the overlay to their own <see cref="IMutableEquipment"/>
        /// implementation; the generated binder calls this during
        /// mount/subscribe with its internal mutable model.
        /// </summary>
        public void ApplyTo(IMutableEquipment model)
        {
            model.Category = Category;
        }
    }
}
