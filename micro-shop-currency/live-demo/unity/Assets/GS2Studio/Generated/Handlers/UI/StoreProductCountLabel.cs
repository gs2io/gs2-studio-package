...
{
    /// <summary>
    /// UI label bound to <c>StoreProduct.count</c>.
    ...
    /// <c>StoreProductListItemHandler</c>; the
    /// handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is
    /// supplied.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/StoreProduct/Label/CountLabel")]
    public sealed class StoreProductCountLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(StoreProductCountLabel)} on '{name}': no StoreProductHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
