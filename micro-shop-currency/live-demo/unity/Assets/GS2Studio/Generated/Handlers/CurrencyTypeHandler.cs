...
{
    /// <summary>
    /// MonoBehaviour handler for CurrencyType. Holds one CurrencyTypeBinder,
    ...
    /// CurrencyTypeListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CurrencyType/CurrencyType Handler")]
    public sealed class CurrencyTypeHandler : CurrencyTypeHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCurrencyTypeBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICurrencyTypeBinderFactory? _binderFactory;

        private ICurrencyTypeBinder? _binder;
        ...
        public override IActionableCurrencyTypeBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CurrencyType"/>, so it is the model.</summary>
        public override CurrencyType? Model => _binder;
        ...
        /// to substitute a fake CurrencyTypeBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICurrencyTypeBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICurrencyTypeBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCurrencyTypeBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CurrencyTypeBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CurrencyTypeId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CurrencyTypeBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CurrencyTypeBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICurrencyTypeBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICurrencyTypeBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICurrencyTypeBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICurrencyTypeBinder binder)
        {
