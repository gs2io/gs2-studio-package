...
{
    /// <summary>
    /// Public read contract for the TriggerPull binder collection: the
    ...
    /// </summary>
    public interface IReadOnlyTriggerPullBinderCollection : IReadOnlyList<IActionableTriggerPullBinder>
    {
        IReadOnlyTriggerPullBinderCollection WhereTrigger(TriggerId trigger);
    }

    /// <summary>
    /// Owning contract for the TriggerPull binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlyTriggerPullBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface ITriggerPullBinderCollection : IReadOnlyTriggerPullBinderCollection, IDisposable
    {
        IComparer<IReadOnlyTriggerPullBinder> Comparer { get; set; }
        event Action<ITriggerPullBinder> ItemAdded;
        event Action<ITriggerPullBinder> ItemRemoved;
        void Invalidate();
        Task MountFromExchangeTriggerPullMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromExchangeTriggerPullMasterData(Action? onChange = null, Action<Exception>? onError = null);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="TriggerPullBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableTriggerPullBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlyTriggerPullBinderCollection : IReadOnlyTriggerPullBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly TriggerPullBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlyTriggerPullBinder>? _predicate;
        private readonly List<IActionableTriggerPullBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableTriggerPullBinder> EmptyList = new List<IActionableTriggerPullBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlyTriggerPullBinderCollection Empty { get; } = new ReadOnlyTriggerPullBinderCollection();
        ...
        /// </summary>
        protected ReadOnlyTriggerPullBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlyTriggerPullBinderCollection(
            TriggerPullBinderCollection rootSource,
            Predicate<IReadOnlyTriggerPullBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableTriggerPullBinder>();
        ...
        public virtual IActionableTriggerPullBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableTriggerPullBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual TriggerPullBinderCollection? GetRootSource() => _rootSource;
        ...
        public IReadOnlyTriggerPullBinderCollection WhereTrigger(TriggerId trigger)
        {
            ThrowIfDisposedForNavigation();
            if (_dead) return Empty;
            if (EqualityComparer<TriggerId>.Default.Equals(trigger, default)) return Empty;
            var root = GetRootSource();
            if (root == null) return Empty;
            var existing = _predicate;
            Predicate<IReadOnlyTriggerPullBinder> composed = existing == null
                ? (Predicate<IReadOnlyTriggerPullBinder>)(b => MatchTrigger(b, trigger))
                : b => existing(b) && MatchTrigger(b, trigger);
            return new ReadOnlyTriggerPullBinderCollection(root, composed);
        }
        private static bool MatchTrigger(IReadOnlyTriggerPullBinder b, TriggerId trigger)
            => EqualityComparer<TriggerId>.Default.Equals(b.Trigger, trigger);
        #endregion

    }

    /// <summary>
    /// Collection binder for TriggerPull.
    /// Wraps a list of TriggerPullBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class TriggerPullBinderCollection : ReadOnlyTriggerPullBinderCollection, ITriggerPullBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;

        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableTriggerPullBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning TriggerPullBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<TriggerPullBinder> _binders = new List<TriggerPullBinder>();
        ...
        private readonly Dictionary<string, TriggerPullBinder> _bindersByRowKey = new Dictionary<string, TriggerPullBinder>();
        ...
        private readonly List<WeakReference<ReadOnlyTriggerPullBinderCollection>> _derivedViews = new List<WeakReference<ReadOnlyTriggerPullBinderCollection>>();
        ...
        private IComparer<IReadOnlyTriggerPullBinder> _configuredComparer = TriggerPullBinderComparer.Default;
        private IComparer<IReadOnlyTriggerPullBinder> _effectiveComparer =
            new TriggerPullBinderIdTieBreakComparer(TriggerPullBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlyTriggerPullBinder so
        /// it never exposes the owning binder.
        /// </summary>
        public IComparer<IReadOnlyTriggerPullBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new TriggerPullBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableTriggerPullBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableTriggerPullBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableTriggerPullBinder contract.
        /// </summary>
        public override IEnumerator<IActionableTriggerPullBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override TriggerPullBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (ITriggerPullBinder).
        // Public so a fake ITriggerPullBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableTriggerPullBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<ITriggerPullBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<ITriggerPullBinder>? ItemRemoved;

        public TriggerPullBinderCollection(
            Gs2Domain gs2,
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlyTriggerPullBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlyTriggerPullBinderCollection>(view));
        }
        ...
        /// <summary>One-shot Create + MountFromExchangeTriggerPullMasterDataAsync (no subscription).</summary>
        public static async Task<TriggerPullBinderCollection> CreateFromExchangeTriggerPullMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new TriggerPullBinderCollection(gs2, session);
            await coll.MountFromExchangeTriggerPullMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromExchangeTriggerPullMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
        ...
        public void SubscribeFromExchangeTriggerPullMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromExchangeTriggerPullMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromExchangeTriggerPullMasterItems(IList<Gs2.Unity.Gs2Exchange.Model.EzRateModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            ...
            {
                if (_disposed) return;
                var rowKey = ExtractExchangeTriggerPullMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyExchangeTriggerPullMasterItemTo(existing.MutableModel, item);
                }
                else
                {
        ...
        private async Task<TriggerPullBinder> BuildBinderFromExchangeTriggerPullMasterItem(Gs2.Unity.Gs2Exchange.Model.EzRateModel item, CancellationToken cancellationToken)
        {
            ...
            var binder = new TriggerPullBinder(model, _gs2, _session);
        ...
        internal sealed class TriggerPullBinderComparer : IComparer<IReadOnlyTriggerPullBinder>
        {
            public static readonly TriggerPullBinderComparer Default = new TriggerPullBinderComparer();
            public int Compare(IReadOnlyTriggerPullBinder? a, IReadOnlyTriggerPullBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<TriggerPullId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class TriggerPullBinderIdTieBreakComparer : IComparer<IReadOnlyTriggerPullBinder>
        {
            private readonly IComparer<IReadOnlyTriggerPullBinder> _inner;
            public TriggerPullBinderIdTieBreakComparer(IComparer<IReadOnlyTriggerPullBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlyTriggerPullBinder? a, IReadOnlyTriggerPullBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
