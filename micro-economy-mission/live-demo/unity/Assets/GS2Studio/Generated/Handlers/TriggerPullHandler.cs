...
{
    /// <summary>
    /// MonoBehaviour handler for TriggerPull. Holds one TriggerPullBinder,
    ...
    /// TriggerPullListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/TriggerPull/TriggerPull Handler")]
    public sealed class TriggerPullHandler : TriggerPullHandlerBase
    {
        ...
        private ITriggerPullBinder? _binder;
        ...
        public override IActionableTriggerPullBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="TriggerPull"/>, so it is the model.</summary>
        public override TriggerPull? Model => _binder;
        ...
        /// to substitute a fake TriggerPullBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a TriggerPullBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new TriggerPullId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned TriggerPullBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a TriggerPullBinderCollection) already
        ...
        /// </summary>
        public void Bind(ITriggerPullBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ITriggerPullBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ITriggerPullBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ITriggerPullBinder binder)
        {
