...
namespace GS2Studio.Generated.Schedule.UI
{
    ...
    /// <c>{id} ({scheduleType})</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Schedule/TemplateLabel/WindowLabel")]
    public sealed class ScheduleWindowLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(ScheduleWindowLabel)} on '{name}': no ScheduleHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.Id.Value} ({model.ScheduleType})");
        }
    }
}
