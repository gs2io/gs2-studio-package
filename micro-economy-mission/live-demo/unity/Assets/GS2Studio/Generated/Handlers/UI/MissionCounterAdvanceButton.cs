...
{
    /// <summary>
    /// UI button bound to <c>Advance</c> on the sibling
    /// <c>MissionCounterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>MissionCounterHandlerBase.Binder.Advance</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>MissionCounterHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/MissionCounter/ButtonAction/AdvanceButton")]
    public sealed class MissionCounterAdvanceButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Advance</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(MissionCounterAdvanceButton)} on '{name}': no MissionCounterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"MissionCounterAdvanceButton: Advance failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"MissionCounterAdvanceButton: Advance failed: {ex}");
                return;
            }
            finally
            {
