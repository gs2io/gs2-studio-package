...
{
    /// <summary>
    /// UI value bound to <c>Trigger.expiresAt</c>.
    /// Subscribes to the sibling <c>TriggerHandler.Updated</c> event
    ...
    /// component alongside (or under) a <c>TriggerHandler</c> or
    /// <c>TriggerListItemHandler</c>; the handler is resolved
    ...
    /// persists it.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Trigger/Value/ExpiresAtValue")]
    public sealed class TriggerExpiresAtValue : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(TriggerExpiresAtValue)} on '{name}': no TriggerHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke(model.ExpiresAt);
        }
