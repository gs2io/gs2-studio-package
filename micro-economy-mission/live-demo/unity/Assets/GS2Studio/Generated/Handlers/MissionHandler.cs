...
{
    /// <summary>
    /// MonoBehaviour handler for Mission. Holds one MissionBinder,
    ...
    /// MissionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Mission/Mission Handler")]
    public sealed class MissionHandler : MissionHandlerBase
    {
        ...
        private IMissionBinder? _binder;
        ...
        public override IActionableMissionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Mission"/>, so it is the model.</summary>
        public override Mission? Model => _binder;
        ...
        /// to substitute a fake MissionBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string missionCollection)
        {
        ...
        /// <summary>
        /// Creates a MissionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new MissionId(_id ?? string.Empty), new MissionCollectionId(_missionCollection ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned MissionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a MissionBinderCollection) already
        ...
        /// </summary>
        public void Bind(IMissionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IMissionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IMissionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IMissionBinder binder)
        {
