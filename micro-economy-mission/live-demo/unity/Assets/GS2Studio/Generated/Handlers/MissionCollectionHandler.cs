...
{
    /// <summary>
    /// MonoBehaviour handler for MissionCollection. Holds one MissionCollectionBinder,
    ...
    /// MissionCollectionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/MissionCollection/MissionCollection Handler")]
    public sealed class MissionCollectionHandler : MissionCollectionHandlerBase
    {
        ...
        private IMissionCollectionBinder? _binder;
        ...
        public override IActionableMissionCollectionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="MissionCollection"/>, so it is the model.</summary>
        public override MissionCollection? Model => _binder;
        ...
        /// to substitute a fake MissionCollectionBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a MissionCollectionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new MissionCollectionId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned MissionCollectionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a MissionCollectionBinderCollection) already
        ...
        /// </summary>
        public void Bind(IMissionCollectionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IMissionCollectionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IMissionCollectionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IMissionCollectionBinder binder)
        {
