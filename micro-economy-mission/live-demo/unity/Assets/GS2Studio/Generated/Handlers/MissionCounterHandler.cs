...
{
    /// <summary>
    /// MonoBehaviour handler for MissionCounter. Holds one MissionCounterBinder,
    ...
    /// MissionCounterListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/MissionCounter/MissionCounter Handler")]
    public sealed class MissionCounterHandler : MissionCounterHandlerBase
    {
        ...
        private IMissionCounterBinder? _binder;
        ...
        public override IActionableMissionCounterBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="MissionCounter"/>, so it is the model.</summary>
        public override MissionCounter? Model => _binder;
        ...
        /// to substitute a fake MissionCounterBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a MissionCounterBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new MissionCounterId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned MissionCounterBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a MissionCounterBinderCollection) already
        ...
        /// </summary>
        public void Bind(IMissionCounterBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IMissionCounterBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IMissionCounterBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IMissionCounterBinder binder)
        {
