// <auto-generated>
//     This code was generated by CSharpHandlerGeneratorService.
//     Do not modify this file directly.
// </auto-generated>

#nullable enable

using System;

using UnityEngine;
using GS2Studio.Generated.Schedule;
using Gs2.Unity.Gs2Mission.Model;
using Gs2Bind.Gs2Mission;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.MissionCollection
{
    /// <summary>
    /// Abstract read surface shared by every handler that exposes a single
    /// MissionCollectionBinder: the standalone/list MissionCollectionHandler and the
    /// slot-bound MissionCollectionListItemHandler. Concrete handlers supply the
    /// binder source; the read-only properties and the Updated/Bound/Failed
    /// events are defined here so consumers can depend on one base type.
    /// </summary>
    public abstract class MissionCollectionHandlerBase : MonoBehaviour
    {
        /// <summary>The bound binder (non-owning actionable view), or null when none is attached.</summary>
        public abstract IActionableMissionCollectionBinder? Binder { get; }

        /// <summary>The bound model, or null when no binder is attached.</summary>
        public abstract MissionCollection? Model { get; }

        /// <summary>True when a binder is currently attached.</summary>
        public abstract bool HasValue { get; }

        /// <summary>Fires whenever the bound model changes (initial attach + each binder update).</summary>
        public event Action<MissionCollection>? Updated;

        /// <summary>Fires once each time a binder is attached (non-owning actionable view).</summary>
        public event Action<IActionableMissionCollectionBinder>? Bound;

        /// <summary>Fires when the handler encounters an exception.</summary>
        public event Action<Exception>? Failed;

        // C# events can only be raised from the declaring type, so derived
        // handlers raise through these protected helpers.
        protected void RaiseUpdated(MissionCollection model) => Updated?.Invoke(model);
        protected void RaiseBound(IActionableMissionCollectionBinder binder) => Bound?.Invoke(binder);
        protected void RaiseFailed(Exception ex) => Failed?.Invoke(ex);
    }
}
