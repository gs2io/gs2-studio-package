...
{
    /// <summary>
    /// MonoBehaviour handler for QuestCollection. Holds one QuestCollectionBinder,
    ...
    /// QuestCollectionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/QuestCollection/QuestCollection Handler")]
    public sealed class QuestCollectionHandler : QuestCollectionHandlerBase
    {
        ...
        private IQuestCollectionBinder? _binder;
        ...
        public override IActionableQuestCollectionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="QuestCollection"/>, so it is the model.</summary>
        public override QuestCollection? Model => _binder;
        ...
        /// to substitute a fake QuestCollectionBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a QuestCollectionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new QuestCollectionId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned QuestCollectionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a QuestCollectionBinderCollection) already
        ...
        /// </summary>
        public void Bind(IQuestCollectionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IQuestCollectionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IQuestCollectionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IQuestCollectionBinder binder)
        {
