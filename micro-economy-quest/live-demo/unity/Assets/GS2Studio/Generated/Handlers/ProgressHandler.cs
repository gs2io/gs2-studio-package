...
{
    /// <summary>
    /// MonoBehaviour handler for Progress. Holds one ProgressBinder,
    ...
    /// ProgressListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Progress/Progress Handler")]
    public sealed class ProgressHandler : ProgressHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultProgressBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IProgressBinderFactory? _binderFactory;

        private IProgressBinder? _binder;
        ...
        public override IActionableProgressBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Progress"/>, so it is the model.</summary>
        public override Progress? Model => _binder;
        ...
        /// to substitute a fake ProgressBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IProgressBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IProgressBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultProgressBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a ProgressBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new ProgressId("progress"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned ProgressBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a ProgressBinderCollection) already
        ...
        /// </summary>
        public void Bind(IProgressBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IProgressBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IProgressBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IProgressBinder binder)
        {
