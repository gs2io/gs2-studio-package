...
{
    /// <summary>
    /// MonoBehaviour handler for StorePrice. Holds one StorePriceBinder,
    ...
    /// StorePriceListItemHandler, which reads its binder from the
    ...
    public sealed class StorePriceHandler : StorePriceHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultStorePriceBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IStorePriceBinderFactory? _binderFactory;

        private IStorePriceBinder? _binder;
        ...
        public override IActionableStorePriceBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="StorePrice"/>, so it is the model.</summary>
        public override StorePrice? Model => _binder;
        ...
        /// to substitute a fake StorePriceBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IStorePriceBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IStorePriceBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultStorePriceBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a StorePriceBinder from the current identity keys, mounts it,
        ...
        /// <summary>
        /// Attaches an externally-owned StorePriceBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a StorePriceBinderCollection) already
        ...
        /// </summary>
        public void Bind(IStorePriceBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IStorePriceBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IStorePriceBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IStorePriceBinder binder)
        {
