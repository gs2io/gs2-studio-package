...
{
    /// <summary>
    /// MonoBehaviour handler for Quest. Holds one QuestBinder,
    ...
    /// QuestListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Quest/Quest Handler")]
    public sealed class QuestHandler : QuestHandlerBase
    {
        ...
        public override IActionableQuestBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Quest"/>, so it is the model.</summary>
        public override Quest? Model => _binder;
        ...
        /// to substitute a fake QuestBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string collection)
        {
        ...
        /// <summary>
        /// Creates a QuestBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new QuestId(_id ?? string.Empty), new QuestCollectionId(_collection ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned QuestBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a QuestBinderCollection) already
        ...
        /// </summary>
        public void Bind(IQuestBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IQuestBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IQuestBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IQuestBinder binder)
        {
