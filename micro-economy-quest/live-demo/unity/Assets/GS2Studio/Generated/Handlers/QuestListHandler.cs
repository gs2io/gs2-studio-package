...
{
    /// <summary>
    /// MonoBehaviour list handler for Quest. Owns a
    /// QuestBinderCollection, instantiates a child QuestListItemHandler
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Quest/Quest List Handler")]
    public sealed class QuestListHandler : MonoBehaviour, IQuestBinderListSource
    {
        ...
        /// itself is the IReadOnlyList&lt;IActionableQuestBinder&gt;.</summary>
        public IReadOnlyList<IActionableQuestBinder> Binders =>
            (IReadOnlyList<IActionableQuestBinder>?)_collectionInternal ?? Array.Empty<IActionableQuestBinder>();
        ...
        private IQuestBinderFactory ResolveBinderFactory()
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            var operationGeneration = ++_reloadGeneration;
            IQuestBinderCollection? operationCollection = null;
            try
            {
                Cleanup();
                ...
                var collection = ResolveBinderFactory().CreateCollection(gs2, session, new QuestCollectionId(_collection ?? string.Empty));
        ...
        private async void Start()
        {
            ...
            try
            {
                if (_collectionInternal != null) return;
        ...
        private bool IsReadyForReload()
        {
            ...
            if (string.IsNullOrEmpty(_collection)) return false;
            return true;
        }

        private bool IsCurrentOperation(long operationGeneration, IQuestBinderCollection? operationCollection)
            => !_isDestroyed
                && operationGeneration == _reloadGeneration
                && (operationCollection == null
                    ? _collectionInternal == null
                    : ReferenceEquals(_collectionInternal, operationCollection));

        private void AttachCollectionCallbacks(
            long operationGeneration,
            IQuestBinderCollection collection)
        {
            Action<IQuestBinder> itemAdded = binder =>
                OnItemAdded(operationGeneration, collection, binder);
            Action<IQuestBinder> itemRemoved = binder =>
        ...
        private void OnCollectionChanged(long operationGeneration, IQuestBinderCollection collection)
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
        ...
        private int IndexOf(IActionableQuestBinder binder)
        {
            var binders = Binders;
        ...
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
            // The collection is the IReadOnlyList<IActionableQuestBinder>; index it
        ...
        private void Cleanup()
        {
            ...
            var handlers = new List<QuestListItemHandler>(_handlers.Values);
            _collectionInternal = null;
