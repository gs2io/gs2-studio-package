...
namespace GS2Studio.Generated.Quest.UI
{
    ...
    /// <c>Running: {displayName} ({staminaCost} stamina spent)</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Quest/TemplateLabel/RunningLabel")]
    public sealed class QuestRunningLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(QuestRunningLabel)} on '{name}': no QuestHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"Running: {model.DisplayName} ({model.StaminaCost} stamina spent)");
        }
    }
}
