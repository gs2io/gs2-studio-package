...
{
    /// <summary>
    /// UI button bound to <c>Start</c> on the sibling
    /// <c>QuestHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>QuestHandlerBase.Binder.Start</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>QuestHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Quest/ButtonAction/StartButton")]
    public sealed class QuestStartButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Start</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(QuestStartButton)} on '{name}': no QuestHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"QuestStartButton: Start failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"QuestStartButton: Start failed: {ex}");
                return;
            }
            finally
            {
