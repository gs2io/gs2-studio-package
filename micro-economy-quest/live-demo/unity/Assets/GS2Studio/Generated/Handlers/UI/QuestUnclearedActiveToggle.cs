...
namespace GS2Studio.Generated.Quest.UI
{
    ...
    /// based on the condition <c>not(truthy(completed))</c> evaluated
    /// against the sibling <c>QuestHandler.Model</c>:
    /// <list type="bullet">
    ...
    /// <c>QuestHandler</c> or <c>QuestListItemHandler</c>;
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Quest/ActiveToggle/UnclearedActiveToggle")]
    public sealed class QuestUnclearedActiveToggle : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(QuestUnclearedActiveToggle)} on '{name}': no QuestHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            bool result = !(((object?)model.Completed) switch { null => false, bool __b => __b, string __s => !string.IsNullOrEmpty(__s), int __i => __i != 0, long __l => __l != 0L, float __f => __f != 0f, double __d => __d != 0d, _ => true });
