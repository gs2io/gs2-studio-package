...
namespace GS2Studio.Generated.Progress.UI
{
    ...
    /// <c>No quest in progress</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>Progress</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Progress/TemplateLabel/IdleLabel")]
    public sealed class ProgressIdleLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(ProgressIdleLabel)} on '{name}': no ProgressHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"No quest in progress");
        }
    }
}
