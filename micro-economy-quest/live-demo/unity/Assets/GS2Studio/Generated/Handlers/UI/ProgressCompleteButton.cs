...
{
    /// <summary>
    /// UI button bound to <c>Complete</c> on the sibling
    /// <c>ProgressHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>ProgressHandlerBase.Binder.Complete</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>ProgressHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Progress/ButtonAction/CompleteButton")]
    public sealed class ProgressCompleteButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Complete</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(ProgressCompleteButton)} on '{name}': no ProgressHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"ProgressCompleteButton: Complete failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"ProgressCompleteButton: Complete failed: {ex}");
                return;
            }
            finally
            {
