...
{
    /// <summary>
    /// Pure read contract for <see cref="GachaRarityRateBinder"/>: the read-only
    /// model surface (<see cref="GachaRarityRate"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyGachaRarityRateBinder : GachaRarityRate
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="GachaRarityRateBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableGachaRarityRateBinder : IReadOnlyGachaRarityRateBinder
    {
    }

    /// <summary>
    /// Owning contract for <see cref="GachaRarityRateBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableGachaRarityRateBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface IGachaRarityRateBinder : IActionableGachaRarityRateBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="GachaRarityRateBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="GachaRarityRate"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyGachaRarityRateBinder : IReadOnlyGachaRarityRateBinder
    {
        ...
        private protected readonly MutableGachaRarityRate _model;
        ...
        /// the read-only <c>GachaRarityRate</c> surface this binder implements.
        /// </summary>
        internal MutableGachaRarityRate MutableModel => _model;
        /// <inheritdoc cref="GachaRarityRate.Id" />
        public GachaRarityRateId Id => _model.Id;
        /// <inheritdoc cref="GachaRarityRate.Gacha" />
        public GachaId Gacha => _model.Gacha;
        /// <inheritdoc cref="GachaRarityRate.Rarity" />
        public GachaRarityId Rarity => _model.Rarity;
        /// <inheritdoc cref="GachaRarityRate.Weight" />
        ...
        /// </summary>
        private protected ReadOnlyGachaRarityRateBinder(MutableGachaRarityRate model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyGachaRarityRateBinder"/>) and
    ...
    /// <summary>
    /// Binder for GachaRarityRate
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class GachaRarityRateBinder : ReadOnlyGachaRarityRateBinder, IGachaRarityRateBinder
    {
        ...
        /// </summary>
        internal GachaRarityRateBinder(
            MutableGachaRarityRate model,
            Gs2Domain gs2,
        ...
        /// <summary>
        /// Creates a GachaRarityRate instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableGachaRarityRate CreateModel(GachaRarityRateId id, GachaId gacha, GachaRarityId rarity)
        {
            var model = new MutableGachaRarityRate();
            model.Id = id;
            model.Gacha = gacha;
            model.Rarity = rarity;
            return model;
        ...
        /// <summary>
        /// Creates a GachaRarityRateBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<GachaRarityRateBinder> CreateAsync(
            GachaId gacha, GachaRarityId rarity,
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var id = new GachaRarityRateId($"{gacha.ToString()}.{rarity.ToString()}");
            var model = CreateModel(id, gacha, rarity);
            var binder = new GachaRarityRateBinder(model, gs2, session);
        ...
        public async Task MountAsync(CancellationToken cancellationToken = default)
        {
            ...
            GachaRarityRateOverlayLoader.Active?.Get(_model.Id.ToString())?.ApplyTo(_model);
            _mounted = true;
        }
        ...
        /// (callable through <see cref="IReadOnlyGachaRarityRateBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(GachaRarityRateBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
