...
namespace GS2Studio.Generated.Gacha.UI
{
    /// <summary>
    /// UI button bound to <c>Buy</c> on the sibling
    /// <c>GachaHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>GachaHandlerBase.Binder.Buy</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>GachaHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Gacha/ButtonAction/BuyButton")]
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(GachaBuyButton)} on '{name}': no GachaHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"GachaBuyButton: Buy failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"GachaBuyButton: Buy failed: {ex}");
                return;
            }
            finally
            {
