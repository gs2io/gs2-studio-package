...
namespace GS2Studio.Generated.Trigger.UI
{
    ...
    /// <c>{triggeredAt} - {expiresAt}</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Trigger/TemplateLabel/WindowLabel")]
    public sealed class TriggerWindowLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(TriggerWindowLabel)} on '{name}': no TriggerHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
