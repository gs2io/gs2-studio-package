...
namespace GS2Studio.Generated.StoreProduct.UI
{
    /// <summary>
    /// UI value bound to <c>StoreProduct.count</c>.
    /// Subscribes to the sibling <c>StoreProductHandler.Updated</c> event
    /// and publishes the resolved <c>int</c> value through
    ...
    /// code to any <c>UnityEvent&lt;int&gt;</c> listener. Add this
    /// component alongside (or under) a <c>StoreProductHandler</c> or
    /// <c>StoreProductListItemHandler</c>; the handler is resolved
    ...
    /// persists it.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/StoreProduct/Value/CountValue")]
    public sealed class StoreProductCountValue : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private StoreProductHandlerBase? _handler;
        [SerializeField] private UnityEvent<int> _onUpdate = new UnityEvent<int>();
        [SerializeField] private UnityEvent<int> _onSelect = new UnityEvent<int>();

        public UnityEvent<int> OnUpdate => _onUpdate;
        public UnityEvent<int> OnSelect => _onSelect;
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(StoreProductCountValue)} on '{name}': no StoreProductHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void OnUpdated(StoreProduct model)
        {
