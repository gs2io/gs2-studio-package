...
{
    /// <summary>
    /// MonoBehaviour handler for GachaRarity. Holds one GachaRarityBinder,
    ...
    /// GachaRarityListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/GachaRarity/GachaRarity Handler")]
    public sealed class GachaRarityHandler : GachaRarityHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultGachaRarityBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IGachaRarityBinderFactory? _binderFactory;

        private IGachaRarityBinder? _binder;
        ...
        public override IActionableGachaRarityBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="GachaRarity"/>, so it is the model.</summary>
        public override GachaRarity? Model => _binder;
        ...
        /// to substitute a fake GachaRarityBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IGachaRarityBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IGachaRarityBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultGachaRarityBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a GachaRarityBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new GachaRarityId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned GachaRarityBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a GachaRarityBinderCollection) already
        ...
        /// </summary>
        public void Bind(IGachaRarityBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IGachaRarityBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IGachaRarityBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IGachaRarityBinder binder)
        {
