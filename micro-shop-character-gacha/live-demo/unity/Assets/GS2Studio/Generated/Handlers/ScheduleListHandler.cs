...
{
    /// <summary>
    /// MonoBehaviour list handler for Schedule. Owns a
    /// ScheduleBinderCollection, instantiates a child ScheduleListItemHandler
    ...
    /// IScheduleBinderListSource so items read Binders[Index] through the
    /// shared parent-list surface.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Schedule/Schedule List Handler")]
    public sealed class ScheduleListHandler : MonoBehaviour, IScheduleBinderListSource
    {
        [SerializeField] private ScheduleListItemHandler? _itemPrefab;
        ...
        // Construction seam. Defaults to DefaultScheduleBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake collection.
        private IScheduleBinderFactory? _binderFactory;

        private IScheduleBinderCollection? _collection;
        ...
        private Action<IScheduleBinder>? _collectionItemAddedCallback;
        private Action<IScheduleBinder>? _collectionItemRemovedCallback;
        ...
        private readonly Dictionary<IActionableScheduleBinder, ScheduleListItemHandler> _handlers =
            new Dictionary<IActionableScheduleBinder, ScheduleListItemHandler>();
        ...
        private IComparer<IReadOnlyScheduleBinder>? _comparer;
        ...
        /// itself is the IReadOnlyList&lt;IActionableScheduleBinder&gt;.</summary>
        public IReadOnlyList<IActionableScheduleBinder> Binders =>
            (IReadOnlyList<IActionableScheduleBinder>?)_collection ?? Array.Empty<IActionableScheduleBinder>();
        ...
        public event Action<IActionableScheduleBinder>? ItemAdded;

        /// <summary>Fires once for each binder removed.</summary>
        public event Action<IActionableScheduleBinder>? ItemRemoved;
        ...
        public IComparer<IReadOnlyScheduleBinder>? Comparer
        {
            ...
            set
            {
                ...
                // ScheduleBinderCollection.ScheduleBinderComparer.
                var applied = value ?? ScheduleBinderCollection.ScheduleBinderComparer.Default;
        ...
        /// to substitute a fake ScheduleBinderCollection; takes precedence over
        /// the default factory. Pass null to revert to the default. Applies to
        /// the next ReloadAsync.
        /// </summary>
        public void SetBinderFactory(IScheduleBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IScheduleBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultScheduleBinderFactory.Instance;
        ...
        /// (the ScheduleBinderCollection only re-sorts on list-membership
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            var operationGeneration = ++_reloadGeneration;
            IScheduleBinderCollection? operationCollection = null;
            try
            {
                Cleanup();
                ...
                await collection.MountFromScheduleScheduleMasterDataAsync(cancellationToken);
                ...
                if (_autoSubscribe)
                {
                    ...
                    collection.SubscribeFromScheduleScheduleMasterData(collectionChanged, collectionFailed);
                }
        ...
        private bool IsCurrentOperation(long operationGeneration, IScheduleBinderCollection? operationCollection)
            => !_isDestroyed
        ...
        {
            Action<IScheduleBinder> itemAdded = binder =>
                OnItemAdded(operationGeneration, collection, binder);
            Action<IScheduleBinder> itemRemoved = binder =>
        ...
        private void OnCollectionChanged(long operationGeneration, IScheduleBinderCollection collection)
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
        ...
        private int IndexOf(IActionableScheduleBinder binder)
        {
            var binders = Binders;
        ...
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
            // The collection is the IReadOnlyList<IActionableScheduleBinder>; index it
        ...
        private void Cleanup()
        {
            ...
            var handlers = new List<ScheduleListItemHandler>(_handlers.Values);
            _collection = null;
