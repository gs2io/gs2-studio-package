...
{
    /// <summary>
    /// MonoBehaviour handler for Character. Holds one CharacterBinder,
    ...
    /// CharacterListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/Character Handler")]
    public sealed class CharacterHandler : CharacterHandlerBase
    {
        ...
        private ICharacterBinder? _binder;
        ...
        /// binder implements <see cref="Character"/>, so it is the model.</summary>
        ...
        /// to substitute a fake CharacterBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string propertyId)
        {
        ...
        /// <summary>
        /// Creates a CharacterBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterId(_id ?? string.Empty), _propertyId, gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterBinder binder)
        {
