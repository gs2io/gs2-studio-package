...
{
    /// <summary>
    /// MonoBehaviour handler for Trigger. Holds one TriggerBinder,
    ...
    /// TriggerListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Trigger/Trigger Handler")]
    public sealed class TriggerHandler : TriggerHandlerBase
    {
        ...
        private ITriggerBinder? _binder;
        ...
        public override IActionableTriggerBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Trigger"/>, so it is the model.</summary>
        public override Trigger? Model => _binder;
        ...
        /// to substitute a fake TriggerBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a TriggerBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new TriggerId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned TriggerBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a TriggerBinderCollection) already
        ...
        /// </summary>
        public void Bind(ITriggerBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ITriggerBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ITriggerBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ITriggerBinder binder)
        {
