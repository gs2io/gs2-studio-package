...
namespace GS2Studio.Generated.GachaRarityRate
{
    ...
    /// GachaRarityRateBinder: the standalone/list GachaRarityRateHandler and the
    /// slot-bound GachaRarityRateListItemHandler. Concrete handlers supply the
    ...
    /// </summary>
    public abstract class GachaRarityRateHandlerBase : MonoBehaviour
    {
        /// <summary>The bound binder (non-owning actionable view), or null when none is attached.</summary>
        public abstract IActionableGachaRarityRateBinder? Binder { get; }

        /// <summary>The bound model, or null when no binder is attached.</summary>
        public abstract GachaRarityRate? Model { get; }
        ...
        public event Action<GachaRarityRate>? Updated;

        /// <summary>Fires once each time a binder is attached (non-owning actionable view).</summary>
        public event Action<IActionableGachaRarityRateBinder>? Bound;
        ...
        protected void RaiseUpdated(GachaRarityRate model) => Updated?.Invoke(model);
        protected void RaiseBound(IActionableGachaRarityRateBinder binder) => Bound?.Invoke(binder);
