...
{
    /// <summary>
    /// MonoBehaviour handler for GachaRarityRate. Holds one GachaRarityRateBinder,
    ...
    /// GachaRarityRateListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/GachaRarityRate/GachaRarityRate Handler")]
    public sealed class GachaRarityRateHandler : GachaRarityRateHandlerBase
    {
        [SerializeField] private string? _gacha;
        ...
        // Construction seam. Defaults to DefaultGachaRarityRateBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IGachaRarityRateBinderFactory? _binderFactory;

        private IGachaRarityRateBinder? _binder;
        ...
        public override IActionableGachaRarityRateBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="GachaRarityRate"/>, so it is the model.</summary>
        public override GachaRarityRate? Model => _binder;
        ...
        /// to substitute a fake GachaRarityRateBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IGachaRarityRateBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IGachaRarityRateBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultGachaRarityRateBinderFactory.Instance;
        ...
        /// </summary>
        public void SetKeys(GachaId gacha, GachaRarityId rarity)
        {
        ...
        /// </summary>
        public void SetKeys(string gacha, string rarity)
        {
            InvalidateBindingIntent();
            _gacha = gacha;
            _rarity = rarity;
        ...
        /// <summary>
        /// Creates a GachaRarityRateBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_gacha))
                    throw new InvalidOperationException("Composite key part '_gacha' is not set.");
                if (string.IsNullOrEmpty(_rarity))
                    throw new InvalidOperationException("Composite key part '_rarity' is not set.");
                var binder = await ResolveBinderFactory().CreateAsync(new GachaId(_gacha ?? string.Empty), new GachaRarityId(_rarity ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned GachaRarityRateBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a GachaRarityRateBinderCollection) already
        ...
        /// </summary>
        public void Bind(IGachaRarityRateBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private bool IsReadyForReload()
        {
            ...
            if (string.IsNullOrEmpty(_gacha)) return false;
            if (string.IsNullOrEmpty(_rarity)) return false;
            return true;
        }
        ...
        private void Attach(IGachaRarityRateBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IGachaRarityRateBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IGachaRarityRateBinder binder)
        {
