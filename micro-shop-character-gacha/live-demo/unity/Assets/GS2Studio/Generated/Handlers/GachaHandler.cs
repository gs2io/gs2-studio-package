...
{
    /// <summary>
    /// MonoBehaviour handler for Gacha. Holds one GachaBinder,
    ...
    /// GachaListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Gacha/Gacha Handler")]
    public sealed class GachaHandler : GachaHandlerBase
    {
        ...
        /// binder implements <see cref="Gacha"/>, so it is the model.</summary>
        public override Gacha? Model => _binder;
        ...
        /// to substitute a fake GachaBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a GachaBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new GachaId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned GachaBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a GachaBinderCollection) already
        ...
        /// </summary>
        public void Bind(IGachaBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IGachaBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IGachaBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IGachaBinder binder)
        {
