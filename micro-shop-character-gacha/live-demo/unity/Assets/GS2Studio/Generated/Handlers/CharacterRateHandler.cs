...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterRate. Holds one CharacterRateBinder,
    ...
    /// CharacterRateListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterRate/CharacterRate Handler")]
    public sealed class CharacterRateHandler : CharacterRateHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterRateBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterRateBinderFactory? _binderFactory;

        private ICharacterRateBinder? _binder;
        ...
        public override IActionableCharacterRateBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterRate"/>, so it is the model.</summary>
        public override CharacterRate? Model => _binder;
        ...
        /// to substitute a fake CharacterRateBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterRateBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterRateBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterRateBinderFactory.Instance;
        ...
        /// </summary>
        public void SetKeys(GachaRarityId rarity, CharacterId character)
        {
        ...
        /// </summary>
        public void SetKeys(string rarity, string character)
        {
            InvalidateBindingIntent();
            _rarity = rarity;
        ...
        /// <summary>
        /// Creates a CharacterRateBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_character))
                    throw new InvalidOperationException("Composite key part '_character' is not set.");
                var binder = await ResolveBinderFactory().CreateAsync(new GachaRarityId(_rarity ?? string.Empty), new CharacterId(_character ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterRateBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterRateBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterRateBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private bool IsReadyForReload()
        {
            ...
            if (string.IsNullOrEmpty(_character)) return false;
            return true;
        }
        ...
        private void Attach(ICharacterRateBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterRateBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterRateBinder binder)
        {
