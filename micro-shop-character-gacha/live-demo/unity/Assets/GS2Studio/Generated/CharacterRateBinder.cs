...
{
    /// <summary>
    /// Pure read contract for <see cref="CharacterRateBinder"/>: the read-only
    /// model surface (<see cref="CharacterRate"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyCharacterRateBinder : CharacterRate
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="CharacterRateBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableCharacterRateBinder : IReadOnlyCharacterRateBinder
    {
    }

    /// <summary>
    /// Owning contract for <see cref="CharacterRateBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableCharacterRateBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface ICharacterRateBinder : IActionableCharacterRateBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="CharacterRateBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="CharacterRate"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyCharacterRateBinder : IReadOnlyCharacterRateBinder
    {
        ...
        private protected readonly MutableCharacterRate _model;
        ...
        /// the read-only <c>CharacterRate</c> surface this binder implements.
        /// </summary>
        internal MutableCharacterRate MutableModel => _model;
        /// <inheritdoc cref="CharacterRate.Id" />
        public CharacterRateId Id => _model.Id;
        /// <inheritdoc cref="CharacterRate.Rarity" />
        public GachaRarityId Rarity => _model.Rarity;
        /// <inheritdoc cref="CharacterRate.Character" />
        public CharacterId Character => _model.Character;
        /// <inheritdoc cref="CharacterRate.Weight" />
        ...
        /// </summary>
        private protected ReadOnlyCharacterRateBinder(MutableCharacterRate model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyCharacterRateBinder"/>) and
    ...
    /// <summary>
    /// Binder for CharacterRate
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class CharacterRateBinder : ReadOnlyCharacterRateBinder, ICharacterRateBinder
    {
        ...
        /// </summary>
        internal CharacterRateBinder(
            MutableCharacterRate model,
            Gs2Domain gs2,
        ...
        /// <summary>
        /// Creates a CharacterRate instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableCharacterRate CreateModel(CharacterRateId id, GachaRarityId rarity, CharacterId character)
        {
            var model = new MutableCharacterRate();
            model.Id = id;
            model.Rarity = rarity;
        ...
        /// <summary>
        /// Creates a CharacterRateBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<CharacterRateBinder> CreateAsync(
            GachaRarityId rarity, CharacterId character,
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var id = new CharacterRateId($"{rarity.ToString()}.{character.ToString()}");
            var model = CreateModel(id, rarity, character);
            var binder = new CharacterRateBinder(model, gs2, session);
        ...
        public async Task MountAsync(CancellationToken cancellationToken = default)
        {
            ...
            CharacterRateOverlayLoader.Active?.Get(_model.Id.ToString())?.ApplyTo(_model);
            _mounted = true;
        }
        ...
        /// (callable through <see cref="IReadOnlyCharacterRateBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(CharacterRateBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
