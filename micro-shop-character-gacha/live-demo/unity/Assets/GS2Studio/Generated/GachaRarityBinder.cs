...
{
    /// <summary>
    /// Pure read contract for <see cref="GachaRarityBinder"/>: the read-only
    /// model surface (<see cref="GachaRarity"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyGachaRarityBinder : GachaRarity
    {
        Task<ICharacterRateBinder> GetCharacterRate(CharacterId character, CancellationToken cancellationToken = default);
        Task<IGachaRarityRateBinder> GetGachaRarityRate(GachaId gacha, CancellationToken cancellationToken = default);
    }

    /// <summary>
    /// Actionable contract for <see cref="GachaRarityBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableGachaRarityBinder : IReadOnlyGachaRarityBinder
    {
    }

    /// <summary>
    /// Owning contract for <see cref="GachaRarityBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableGachaRarityBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface IGachaRarityBinder : IActionableGachaRarityBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="GachaRarityBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="GachaRarity"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyGachaRarityBinder : IReadOnlyGachaRarityBinder
    {
        ...
        private protected readonly MutableGachaRarity _model;
        ...
        /// the read-only <c>GachaRarity</c> surface this binder implements.
        /// </summary>
        internal MutableGachaRarity MutableModel => _model;
        /// <inheritdoc cref="GachaRarity.Id" />
        public GachaRarityId Id => _model.Id;
        /// <summary>
        ...
        /// </summary>
        private protected ReadOnlyGachaRarityBinder(MutableGachaRarity model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyGachaRarityBinder"/>) and
        ...
        public async Task<ICharacterRateBinder> GetCharacterRate(CharacterId character, CancellationToken cancellationToken = default)
        {
            ThrowIfDisposedForNavigation();
            return await CharacterRateBinder.CreateAsync(_model.Id, character, _gs2, _session, cancellationToken);
        }

        public async Task<IGachaRarityRateBinder> GetGachaRarityRate(GachaId gacha, CancellationToken cancellationToken = default)
        {
            ThrowIfDisposedForNavigation();
            return await GachaRarityRateBinder.CreateAsync(gacha, _model.Id, _gs2, _session, cancellationToken);
        }
        #endregion
    }

    /// <summary>
    /// Binder for GachaRarity
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class GachaRarityBinder : ReadOnlyGachaRarityBinder, IGachaRarityBinder
    {
        ...
        /// </summary>
        internal GachaRarityBinder(
            MutableGachaRarity model,
            Gs2Domain gs2,
        ...
        /// <summary>
        /// Creates a GachaRarity instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableGachaRarity CreateModel(GachaRarityId id)
        {
            var model = new MutableGachaRarity();
            model.Id = id;
            return model;
        }

        /// <summary>
        /// Creates a GachaRarityBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<GachaRarityBinder> CreateAsync(
            GachaRarityId id,
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            ...
            var binder = new GachaRarityBinder(model, gs2, session);
        ...
        /// (callable through <see cref="IReadOnlyGachaRarityBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(GachaRarityBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
