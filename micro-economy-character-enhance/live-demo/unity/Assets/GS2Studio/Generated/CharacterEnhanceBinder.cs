...
{
    /// <summary>
    /// Pure read contract for <see cref="CharacterEnhanceBinder"/>: the read-only
    /// model surface (<see cref="CharacterEnhance"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyCharacterEnhanceBinder : CharacterEnhance
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="CharacterEnhanceBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableCharacterEnhanceBinder : IReadOnlyCharacterEnhanceBinder
    {
    }

    /// <summary>
    /// Owning contract for <see cref="CharacterEnhanceBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableCharacterEnhanceBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface ICharacterEnhanceBinder : IActionableCharacterEnhanceBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="CharacterEnhanceBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="CharacterEnhance"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyCharacterEnhanceBinder : IReadOnlyCharacterEnhanceBinder
    {
        ...
        private protected readonly MutableCharacterEnhance _model;
        ...
        /// the read-only <c>CharacterEnhance</c> surface this binder implements.
        /// </summary>
        internal MutableCharacterEnhance MutableModel => _model;
        /// <inheritdoc cref="CharacterEnhance.Id" />
        public CharacterEnhanceId Id => _model.Id;
        /// <summary>
        ...
        /// </summary>
        private protected ReadOnlyCharacterEnhanceBinder(MutableCharacterEnhance model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyCharacterEnhanceBinder"/>) and
    ...
    /// <summary>
    /// Binder for CharacterEnhance
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class CharacterEnhanceBinder : ReadOnlyCharacterEnhanceBinder, ICharacterEnhanceBinder
    {
        private readonly List<Action> _unsubscribers = new List<Action>();
        private bool _disposed;
        internal bool _mounted;

        private readonly Gs2Bind.Gs2Enhance.RateModelLoader __enhanceCharacterEnhanceNamespaceRateModelLoader;
        ...
        /// </summary>
        internal CharacterEnhanceBinder(
            MutableCharacterEnhance model,
            Gs2Domain gs2,
            IGameSession session
        ) : base(model, gs2, session)
        {
            __enhanceCharacterEnhanceNamespaceRateModelLoader = new Gs2Bind.Gs2Enhance.RateModelLoader("CharacterEnhance", _model.Id);
        }

        /// <summary>
        /// Creates a CharacterEnhance instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableCharacterEnhance CreateModel(CharacterEnhanceId id)
        {
            var model = new MutableCharacterEnhance();
            model.Id = id;
            return model;
        }

        /// <summary>
        /// Creates a CharacterEnhanceBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<CharacterEnhanceBinder> CreateAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var binder = new CharacterEnhanceBinder(model, gs2, session);
        ...
        public async Task MountAsync(CancellationToken cancellationToken = default)
        {
            ...
            var _enhanceCharacterEnhanceNamespaceRateModel = await __enhanceCharacterEnhanceNamespaceRateModelLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            if (_enhanceCharacterEnhanceNamespaceRateModel != null)
            {
            }
            _mounted = true;
        }
        ...
        {
            ThrowIfDisposed();
            _unsubscribers.Add(__enhanceCharacterEnhanceNamespaceRateModelLoader.Subscribe(
                _gs2,
                _session,
                (_, _, value) =>
                {
        ...
        {
            ThrowIfDisposed();
            __enhanceCharacterEnhanceNamespaceRateModelLoader.Invalidate(_gs2, _session);
        }
        ...
        /// (callable through <see cref="IReadOnlyCharacterEnhanceBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(CharacterEnhanceBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
