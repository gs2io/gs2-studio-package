...
namespace GS2Studio.Generated.CharacterEnhanceMaterial.UI
{
    ...
    /// <c>{id} x{count}</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>CharacterEnhanceMaterial</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterEnhanceMaterial/TemplateLabel/StockLabel")]
    public sealed class CharacterEnhanceMaterialStockLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterEnhanceMaterialStockLabel)} on '{name}': no CharacterEnhanceMaterialHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void OnUpdated(CharacterEnhanceMaterial model)
        {
