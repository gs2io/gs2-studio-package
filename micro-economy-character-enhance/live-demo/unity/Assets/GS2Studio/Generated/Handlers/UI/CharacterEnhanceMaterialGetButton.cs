...
using GS2Studio.Generated.CharacterEnhanceMaterial;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.CharacterEnhanceMaterial.UI
{
    /// <summary>
    /// UI button bound to <c>Get</c> on the sibling
    /// <c>CharacterEnhanceMaterialHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterEnhanceMaterialHandlerBase.Binder.Get</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>CharacterEnhanceMaterialHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterEnhanceMaterial/ButtonAction/GetButton")]
    public sealed class CharacterEnhanceMaterialGetButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private CharacterEnhanceMaterialHandlerBase? _handler;
        ...
        /// in that window is dropped, not queued: a second <c>Get</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<CharacterEnhanceMaterialHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterEnhanceMaterialGetButton)} on '{name}': no CharacterEnhanceMaterialHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"CharacterEnhanceMaterialGetButton: Get failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterEnhanceMaterialGetButton: Get failed: {ex}");
                return;
            }
            finally
            {
