...
{
    /// <summary>
    /// UI button bound to <c>EnhanceWithPotion</c> on the sibling
    /// <c>CharacterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterHandlerBase.Binder.EnhanceWithPotion</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/ButtonAction/EnhanceWithPotionButton")]
    public sealed class CharacterEnhanceWithPotionButton : MonoBehaviour
    {
        ...
        /// <summary>
        /// Loads the arguments of <c>EnhanceWithPotion</c> are read from. A row
        ...
        /// in that window is dropped, not queued: a second <c>EnhanceWithPotion</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterEnhanceWithPotionButton)} on '{name}': no CharacterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.EnhanceWithPotion();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"CharacterEnhanceWithPotionButton: EnhanceWithPotion failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterEnhanceWithPotionButton: EnhanceWithPotion failed: {ex}");
                return;
            }
            finally
            {
        ...
        private void WarnUnloadedLoadersOnce(IActionableCharacterBinder binder)
        {
            ...
            Debug.LogWarning(
                $"{nameof(CharacterEnhanceWithPotionButton)} on '{name}': this row's mount surface did not run UserdataInventoryCharacterItemModel, which fills what this component reads (PropertyId, Id); those readings render as their default, not as an absent value.", this);
        }
    }
}
