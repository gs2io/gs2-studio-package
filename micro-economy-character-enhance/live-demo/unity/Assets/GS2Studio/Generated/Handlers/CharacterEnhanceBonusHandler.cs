...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterEnhanceBonus. Holds one CharacterEnhanceBonusBinder,
    ...
    /// CharacterEnhanceBonusListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterEnhanceBonus/CharacterEnhanceBonus Handler")]
    public sealed class CharacterEnhanceBonusHandler : CharacterEnhanceBonusHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterEnhanceBonusBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterEnhanceBonusBinderFactory? _binderFactory;

        private ICharacterEnhanceBonusBinder? _binder;
        ...
        public override IActionableCharacterEnhanceBonusBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterEnhanceBonus"/>, so it is the model.</summary>
        public override CharacterEnhanceBonus? Model => _binder;
        ...
        /// to substitute a fake CharacterEnhanceBonusBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterEnhanceBonusBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterEnhanceBonusBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterEnhanceBonusBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CharacterEnhanceBonusBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterEnhanceBonusId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterEnhanceBonusBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterEnhanceBonusBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterEnhanceBonusBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterEnhanceBonusBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterEnhanceBonusBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterEnhanceBonusBinder binder)
        {
