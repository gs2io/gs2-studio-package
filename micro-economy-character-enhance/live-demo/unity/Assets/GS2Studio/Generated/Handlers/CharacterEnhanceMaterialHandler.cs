...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterEnhanceMaterial. Holds one CharacterEnhanceMaterialBinder,
    ...
    /// CharacterEnhanceMaterialListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterEnhanceMaterial/CharacterEnhanceMaterial Handler")]
    public sealed class CharacterEnhanceMaterialHandler : CharacterEnhanceMaterialHandlerBase
    {
        ...
        private ICharacterEnhanceMaterialBinder? _binder;
        ...
        public override IActionableCharacterEnhanceMaterialBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterEnhanceMaterial"/>, so it is the model.</summary>
        public override CharacterEnhanceMaterial? Model => _binder;
        ...
        /// to substitute a fake CharacterEnhanceMaterialBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a CharacterEnhanceMaterialBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterEnhanceMaterialId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterEnhanceMaterialBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterEnhanceMaterialBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterEnhanceMaterialBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterEnhanceMaterialBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterEnhanceMaterialBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterEnhanceMaterialBinder binder)
        {
