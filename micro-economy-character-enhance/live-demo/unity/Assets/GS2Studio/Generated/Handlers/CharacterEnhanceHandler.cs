...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterEnhance. Holds one CharacterEnhanceBinder,
    ...
    /// CharacterEnhanceListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterEnhance/CharacterEnhance Handler")]
    public sealed class CharacterEnhanceHandler : CharacterEnhanceHandlerBase
    {
        ...
        private ICharacterEnhanceBinder? _binder;
        ...
        public override IActionableCharacterEnhanceBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterEnhance"/>, so it is the model.</summary>
        public override CharacterEnhance? Model => _binder;
        ...
        /// to substitute a fake CharacterEnhanceBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a CharacterEnhanceBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterEnhanceId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterEnhanceBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterEnhanceBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterEnhanceBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterEnhanceBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterEnhanceBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterEnhanceBinder binder)
        {
