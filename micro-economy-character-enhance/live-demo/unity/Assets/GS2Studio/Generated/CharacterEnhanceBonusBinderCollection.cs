...
{
    /// <summary>
    /// Public read contract for the CharacterEnhanceBonus binder collection: the
    ...
    /// </summary>
    public interface IReadOnlyCharacterEnhanceBonusBinderCollection : IReadOnlyList<IActionableCharacterEnhanceBonusBinder>
    {
        IReadOnlyCharacterEnhanceBonusBinderCollection WhereEnhance(CharacterEnhanceId enhance);
    }

    /// <summary>
    /// Owning contract for the CharacterEnhanceBonus binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlyCharacterEnhanceBonusBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface ICharacterEnhanceBonusBinderCollection : IReadOnlyCharacterEnhanceBonusBinderCollection, IDisposable
    {
        IComparer<IReadOnlyCharacterEnhanceBonusBinder> Comparer { get; set; }
        event Action<ICharacterEnhanceBonusBinder> ItemAdded;
        event Action<ICharacterEnhanceBonusBinder> ItemRemoved;
        void Invalidate();
        Task MountFromOverlayAsync(CancellationToken cancellationToken = default);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="CharacterEnhanceBonusBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableCharacterEnhanceBonusBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlyCharacterEnhanceBonusBinderCollection : IReadOnlyCharacterEnhanceBonusBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly CharacterEnhanceBonusBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlyCharacterEnhanceBonusBinder>? _predicate;
        private readonly List<IActionableCharacterEnhanceBonusBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableCharacterEnhanceBonusBinder> EmptyList = new List<IActionableCharacterEnhanceBonusBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlyCharacterEnhanceBonusBinderCollection Empty { get; } = new ReadOnlyCharacterEnhanceBonusBinderCollection();
        ...
        /// </summary>
        protected ReadOnlyCharacterEnhanceBonusBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlyCharacterEnhanceBonusBinderCollection(
            CharacterEnhanceBonusBinderCollection rootSource,
            Predicate<IReadOnlyCharacterEnhanceBonusBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableCharacterEnhanceBonusBinder>();
        ...
        public virtual IActionableCharacterEnhanceBonusBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableCharacterEnhanceBonusBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual CharacterEnhanceBonusBinderCollection? GetRootSource() => _rootSource;
        ...
        public IReadOnlyCharacterEnhanceBonusBinderCollection WhereEnhance(CharacterEnhanceId enhance)
        {
            ThrowIfDisposedForNavigation();
            if (_dead) return Empty;
            if (EqualityComparer<CharacterEnhanceId>.Default.Equals(enhance, default)) return Empty;
            var root = GetRootSource();
            if (root == null) return Empty;
            var existing = _predicate;
            Predicate<IReadOnlyCharacterEnhanceBonusBinder> composed = existing == null
                ? (Predicate<IReadOnlyCharacterEnhanceBonusBinder>)(b => MatchEnhance(b, enhance))
                : b => existing(b) && MatchEnhance(b, enhance);
            return new ReadOnlyCharacterEnhanceBonusBinderCollection(root, composed);
        }
        private static bool MatchEnhance(IReadOnlyCharacterEnhanceBonusBinder b, CharacterEnhanceId enhance)
            => EqualityComparer<CharacterEnhanceId>.Default.Equals(b.Enhance, enhance);
        #endregion

    }

    /// <summary>
    /// Collection binder for CharacterEnhanceBonus.
    /// Wraps a list of CharacterEnhanceBonusBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class CharacterEnhanceBonusBinderCollection : ReadOnlyCharacterEnhanceBonusBinderCollection, ICharacterEnhanceBonusBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;

        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableCharacterEnhanceBonusBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning CharacterEnhanceBonusBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<CharacterEnhanceBonusBinder> _binders = new List<CharacterEnhanceBonusBinder>();
        ...
        private readonly Dictionary<string, CharacterEnhanceBonusBinder> _bindersByRowKey = new Dictionary<string, CharacterEnhanceBonusBinder>();
        ...
        private IComparer<IReadOnlyCharacterEnhanceBonusBinder> _configuredComparer = CharacterEnhanceBonusBinderComparer.Default;
        private IComparer<IReadOnlyCharacterEnhanceBonusBinder> _effectiveComparer =
            new CharacterEnhanceBonusBinderIdTieBreakComparer(CharacterEnhanceBonusBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlyCharacterEnhanceBonusBinder so
        /// it never exposes the owning binder.
        /// </summary>
        public IComparer<IReadOnlyCharacterEnhanceBonusBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new CharacterEnhanceBonusBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableCharacterEnhanceBonusBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableCharacterEnhanceBonusBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableCharacterEnhanceBonusBinder contract.
        /// </summary>
        public override IEnumerator<IActionableCharacterEnhanceBonusBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override CharacterEnhanceBonusBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (ICharacterEnhanceBonusBinder).
        // Public so a fake ICharacterEnhanceBonusBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableCharacterEnhanceBonusBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<ICharacterEnhanceBonusBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<ICharacterEnhanceBonusBinder>? ItemRemoved;

        public CharacterEnhanceBonusBinderCollection(
            Gs2Domain gs2,
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlyCharacterEnhanceBonusBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlyCharacterEnhanceBonusBinderCollection>(view));
        }
        ...
        public static async Task<CharacterEnhanceBonusBinderCollection> CreateFromOverlayAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new CharacterEnhanceBonusBinderCollection(gs2, session);
        ...
        public async Task MountFromOverlayAsync(CancellationToken cancellationToken = default)
        {
            ...
            var entries = CharacterEnhanceBonusOverlayLoader.Active?.GetAll();
            ...
            foreach (var entry in entries)
            {
                ...
                var id = new CharacterEnhanceBonusId(entry.RawInstanceId);
                if (EqualityComparer<CharacterEnhanceBonusId>.Default.Equals(id, default)) continue;
                seen.Add(rowKey);
                ...
                var model = CharacterEnhanceBonusBinder.CreateModel(id);
                entry.ApplyTo(model);
                var binder = new CharacterEnhanceBonusBinder(model, _gs2, _session);
        ...
        internal sealed class CharacterEnhanceBonusBinderComparer : IComparer<IReadOnlyCharacterEnhanceBonusBinder>
        {
            public static readonly CharacterEnhanceBonusBinderComparer Default = new CharacterEnhanceBonusBinderComparer();
            public int Compare(IReadOnlyCharacterEnhanceBonusBinder? a, IReadOnlyCharacterEnhanceBonusBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<CharacterEnhanceBonusId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class CharacterEnhanceBonusBinderIdTieBreakComparer : IComparer<IReadOnlyCharacterEnhanceBonusBinder>
        {
            private readonly IComparer<IReadOnlyCharacterEnhanceBonusBinder> _inner;
            public CharacterEnhanceBonusBinderIdTieBreakComparer(IComparer<IReadOnlyCharacterEnhanceBonusBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlyCharacterEnhanceBonusBinder? a, IReadOnlyCharacterEnhanceBonusBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
