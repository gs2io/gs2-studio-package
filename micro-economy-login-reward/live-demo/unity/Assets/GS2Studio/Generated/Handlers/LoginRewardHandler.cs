...
{
    /// <summary>
    /// MonoBehaviour handler for LoginReward. Holds one LoginRewardBinder,
    ...
    /// LoginRewardListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginReward/LoginReward Handler")]
    public sealed class LoginRewardHandler : LoginRewardHandlerBase
    {
        ...
        private ILoginRewardBinder? _binder;
        ...
        /// binder implements <see cref="LoginReward"/>, so it is the model.</summary>
        ...
        /// to substitute a fake LoginRewardBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, int rewardIndex, string loginRewardCollection)
        {
        ...
        /// <summary>
        /// Creates a LoginRewardBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new LoginRewardId(_id ?? string.Empty), _rewardIndex, new LoginRewardCollectionId(_loginRewardCollection ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned LoginRewardBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a LoginRewardBinderCollection) already
        ...
        /// </summary>
        public void Bind(ILoginRewardBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ILoginRewardBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ILoginRewardBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ILoginRewardBinder binder)
        {
