...
{
    /// <summary>
    /// MonoBehaviour handler for LoginRewardCollection. Holds one LoginRewardCollectionBinder,
    ...
    /// LoginRewardCollectionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginRewardCollection/LoginRewardCollection Handler")]
    public sealed class LoginRewardCollectionHandler : LoginRewardCollectionHandlerBase
    {
        ...
        private ILoginRewardCollectionBinder? _binder;
        ...
        public override IActionableLoginRewardCollectionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="LoginRewardCollection"/>, so it is the model.</summary>
        public override LoginRewardCollection? Model => _binder;
        ...
        /// to substitute a fake LoginRewardCollectionBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a LoginRewardCollectionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new LoginRewardCollectionId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned LoginRewardCollectionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a LoginRewardCollectionBinderCollection) already
        ...
        /// </summary>
        public void Bind(ILoginRewardCollectionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ILoginRewardCollectionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ILoginRewardCollectionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ILoginRewardCollectionBinder binder)
        {
