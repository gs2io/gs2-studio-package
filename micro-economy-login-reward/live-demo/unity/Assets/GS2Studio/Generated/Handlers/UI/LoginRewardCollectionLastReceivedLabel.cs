...
namespace GS2Studio.Generated.LoginRewardCollection.UI
{
    ...
    /// <c>Last received: {lastReceivedAt} (UTC)</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginRewardCollection/TemplateLabel/LastReceivedLabel")]
    public sealed class LoginRewardCollectionLastReceivedLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardCollectionLastReceivedLabel)} on '{name}': no LoginRewardCollectionHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
