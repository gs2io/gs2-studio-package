...
{
    /// <summary>
    /// UI button bound to <c>MissedReceive</c> on the sibling
    /// <c>LoginRewardCollectionHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>LoginRewardCollectionHandlerBase.Binder.MissedReceive</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>LoginRewardCollectionHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginRewardCollection/ButtonAction/CatchUpButton")]
    public sealed class LoginRewardCollectionCatchUpButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>MissedReceive</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardCollectionCatchUpButton)} on '{name}': no LoginRewardCollectionHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.MissedReceive();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionCatchUpButton: MissedReceive failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionCatchUpButton: MissedReceive failed: {ex}");
                return;
            }
            finally
            {
