...
namespace GS2Studio.Generated.LoginReward.UI
{
    ...
    /// <c>{displayName}: {coins} coins</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>LoginReward</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginReward/TemplateLabel/DayLabel")]
    public sealed class LoginRewardDayLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardDayLabel)} on '{name}': no LoginRewardHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.DisplayName}: {model.Coins} coins");
        }
    }
}
