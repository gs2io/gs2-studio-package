...
{
    /// <summary>
    /// UI button bound to <c>StartOver</c> on the sibling
    /// <c>LoginRewardCollectionHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>LoginRewardCollectionHandlerBase.Binder.StartOver</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginRewardCollection/ButtonAction/StartOverButton")]
    public sealed class LoginRewardCollectionStartOverButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>StartOver</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardCollectionStartOverButton)} on '{name}': no LoginRewardCollectionHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionStartOverButton: StartOver failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionStartOverButton: StartOver failed: {ex}");
                return;
            }
            finally
            {
