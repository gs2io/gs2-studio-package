...
{
    /// <summary>
    /// UI button bound to <c>Receive</c> on the sibling
    /// <c>LoginRewardCollectionHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>LoginRewardCollectionHandlerBase.Binder.Receive</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginRewardCollection/ButtonAction/ReceiveButton")]
    public sealed class LoginRewardCollectionReceiveButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Receive</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardCollectionReceiveButton)} on '{name}': no LoginRewardCollectionHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionReceiveButton: Receive failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"LoginRewardCollectionReceiveButton: Receive failed: {ex}");
                return;
            }
            finally
            {
