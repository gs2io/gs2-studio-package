...
namespace GS2Studio.Generated.LoginReward.UI
{
    ...
    /// based on the condition <c>not(rowElement(LoginRewardCollection.receivedSteps))</c> evaluated
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/LoginReward/ActiveToggle/UnreceivedActiveToggle")]
    public sealed class LoginRewardUnreceivedActiveToggle : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardUnreceivedActiveToggle)} on '{name}': no LoginRewardHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
            ...
            if (_loginRewardCollectionHandler == null)
            {
                ...
                if (!_warnedMissingLoginRewardCollectionHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardUnreceivedActiveToggle)} on '{name}': no LoginRewardCollectionHandlerBase in the parent chain; " +
        ...
        private bool ReadLoginRewardCollectionReceivedStepsAtRow(LoginReward model)
        {
            ...
            if (string.IsNullOrEmpty(parentKey) || !string.Equals(parentKey, companionKey, StringComparison.Ordinal))
            {
                if (!_warnedLoginRewardCollectionReceivedStepsParentMismatch)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(LoginRewardUnreceivedActiveToggle)} on '{name}': this row belongs to LoginRewardCollection '{parentKey}', but the LoginRewardCollectionHandlerBase above it holds '{companionKey}'; reading false instead of another row's receivedSteps.", this);
                }
                return false;
            }
