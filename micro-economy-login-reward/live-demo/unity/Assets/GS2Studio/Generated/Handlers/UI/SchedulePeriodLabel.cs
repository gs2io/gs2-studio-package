...
namespace GS2Studio.Generated.Schedule.UI
{
    ...
    /// <c>{startAt} - {endAt}</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Schedule/TemplateLabel/PeriodLabel")]
    public sealed class SchedulePeriodLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(SchedulePeriodLabel)} on '{name}': no ScheduleHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
