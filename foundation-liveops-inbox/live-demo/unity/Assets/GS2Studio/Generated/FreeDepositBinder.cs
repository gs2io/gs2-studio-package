...
{
    /// <summary>
    /// Pure read contract for <see cref="FreeDepositBinder"/>: the read-only
    /// model surface (<see cref="FreeDeposit"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyFreeDepositBinder : FreeDeposit
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="FreeDepositBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableFreeDepositBinder : IReadOnlyFreeDepositBinder
    {
        Task Deposit(Gs2.Unity.Gs2Exchange.Model.EzConfig[]? config = null);
    }

    /// <summary>
    /// Owning contract for <see cref="FreeDepositBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableFreeDepositBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface IFreeDepositBinder : IActionableFreeDepositBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="FreeDepositBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="FreeDeposit"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyFreeDepositBinder : IReadOnlyFreeDepositBinder
    {
        ...
        private protected readonly MutableFreeDeposit _model;
        ...
        /// the read-only <c>FreeDeposit</c> surface this binder implements.
        /// </summary>
        internal MutableFreeDeposit MutableModel => _model;
        /// <inheritdoc cref="FreeDeposit.Id" />
        public FreeDepositId Id => _model.Id;
        /// <inheritdoc cref="FreeDeposit.Count" />
        ...
        /// </summary>
        private protected ReadOnlyFreeDepositBinder(MutableFreeDeposit model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyFreeDepositBinder"/>) and
    ...
    /// <summary>
    /// Binder for FreeDeposit
    /// Binds GS2 resource values to the model and automatically reflects server-side changes
    /// </summary>
    public sealed class FreeDepositBinder : ReadOnlyFreeDepositBinder, IFreeDepositBinder
    {
        ...
        /// </summary>
        internal FreeDepositBinder(
            MutableFreeDeposit model,
            Gs2Domain gs2,
        ...
        /// <summary>
        /// Creates a FreeDeposit instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableFreeDeposit CreateModel(FreeDepositId id)
        {
            var model = new MutableFreeDeposit();
            model.Id = id;
            return model;
        }

        /// <summary>
        /// Creates a FreeDepositBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<FreeDepositBinder> CreateAsync(
            FreeDepositId id,
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            ...
            var binder = new FreeDepositBinder(model, gs2, session);
        ...
        /// (callable through <see cref="IReadOnlyFreeDepositBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(FreeDepositBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
        ...
        public static bool RestoreCount(IMutableFreeDeposit model, string actionName, string requestJson)
        {
