...
{
    /// <summary>
    /// Public read contract for the Message binder collection: the
    ...
    /// </summary>
    public interface IReadOnlyMessageBinderCollection : IReadOnlyList<IActionableMessageBinder>
    {
    }

    /// <summary>
    /// Owning contract for the Message binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlyMessageBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface IMessageBinderCollection : IReadOnlyMessageBinderCollection, IDisposable
    {
        IComparer<IReadOnlyMessageBinder> Comparer { get; set; }
        event Action<IMessageBinder> ItemAdded;
        event Action<IMessageBinder> ItemRemoved;
        void Invalidate();
        Task MountFromInboxInboxUserDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromInboxInboxUserData(Action? onChange = null, Action<Exception>? onError = null);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="MessageBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableMessageBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlyMessageBinderCollection : IReadOnlyMessageBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly MessageBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlyMessageBinder>? _predicate;
        private readonly List<IActionableMessageBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableMessageBinder> EmptyList = new List<IActionableMessageBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlyMessageBinderCollection Empty { get; } = new ReadOnlyMessageBinderCollection();
        ...
        /// </summary>
        protected ReadOnlyMessageBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlyMessageBinderCollection(
            MessageBinderCollection rootSource,
            Predicate<IReadOnlyMessageBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableMessageBinder>();
        ...
        public virtual IActionableMessageBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableMessageBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual MessageBinderCollection? GetRootSource() => _rootSource;
    ...
    /// <summary>
    /// Collection binder for Message.
    /// Wraps a list of MessageBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class MessageBinderCollection : ReadOnlyMessageBinderCollection, IMessageBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;

        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableMessageBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning MessageBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<MessageBinder> _binders = new List<MessageBinder>();
        ...
        private readonly Dictionary<string, MessageBinder> _bindersByRowKey = new Dictionary<string, MessageBinder>();
        ...
        private readonly List<WeakReference<ReadOnlyMessageBinderCollection>> _derivedViews = new List<WeakReference<ReadOnlyMessageBinderCollection>>();
        ...
        private IComparer<IReadOnlyMessageBinder> _configuredComparer = MessageBinderComparer.Default;
        private IComparer<IReadOnlyMessageBinder> _effectiveComparer =
            new MessageBinderIdTieBreakComparer(MessageBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlyMessageBinder so
        /// it never exposes the owning binder.
        /// </summary>
        public IComparer<IReadOnlyMessageBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new MessageBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableMessageBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableMessageBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableMessageBinder contract.
        /// </summary>
        public override IEnumerator<IActionableMessageBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override MessageBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (IMessageBinder).
        // Public so a fake IMessageBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableMessageBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<IMessageBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<IMessageBinder>? ItemRemoved;

        public MessageBinderCollection(
            Gs2Domain gs2,
        ...
        {
            ThrowIfDisposed();
            new Gs2Bind.Gs2Inbox.MessageArrayLoader("Inbox").Invalidate(_gs2, _session);
        }
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlyMessageBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlyMessageBinderCollection>(view));
        }
        ...
        /// <summary>One-shot Create + MountFromInboxInboxUserDataAsync (no subscription).</summary>
        public static async Task<MessageBinderCollection> CreateFromInboxInboxUserDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new MessageBinderCollection(gs2, session);
            await coll.MountFromInboxInboxUserDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromInboxInboxUserDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
            var arrayLoader = new Gs2Bind.Gs2Inbox.MessageArrayLoader("Inbox");
            var items = await arrayLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            await ReconcileFromInboxInboxUserItems(items, attachChildSubscribe: false, cancellationToken);
            _mounted = true;
        }

        public void SubscribeFromInboxInboxUserData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromInboxInboxUserItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromInboxInboxUserItems(IList<Gs2.Unity.Gs2Inbox.Model.EzMessage> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            var seen = new HashSet<string>();
            foreach (var item in items)
            {
                if (_disposed) return;
                var rowKey = ExtractInboxInboxUserRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyInboxInboxUserItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromInboxInboxUserItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    {
                        ApplyInboxInboxUserItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
        ...
        private async Task<MessageBinder> BuildBinderFromInboxInboxUserItem(Gs2.Unity.Gs2Inbox.Model.EzMessage item, CancellationToken cancellationToken)
        {
            var model = MessageBinder.CreateModel((string.IsNullOrEmpty(item.Name) ? default(MessageId) : new MessageId(item.Name)));
            ApplyInboxInboxUserItemTo(model, item);
            var binder = new MessageBinder(model, _gs2, _session);
            await binder.MountAsync(cancellationToken);
            return binder;
        }

        private static void ApplyInboxInboxUserItemTo(MutableMessage model, Gs2.Unity.Gs2Inbox.Model.EzMessage item)
        {
            MessageBinder.ApplyUserdataInboxMessage(model, item);
        }

        private MessageId ExtractInboxInboxUserIdentity(Gs2.Unity.Gs2Inbox.Model.EzMessage item)
        {
            return (string.IsNullOrEmpty(item.Name) ? default(MessageId) : new MessageId(item.Name));
        }

        private string? ExtractInboxInboxUserRowKey(Gs2.Unity.Gs2Inbox.Model.EzMessage item)
        {
            var id = ExtractInboxInboxUserIdentity(item);
            if (EqualityComparer<MessageId>.Default.Equals(id, default)) return null;
            if (string.IsNullOrEmpty(item.Name)) return null;
            return $"{item.Name}";
        }

        internal sealed class MessageBinderComparer : IComparer<IReadOnlyMessageBinder>
        {
            public static readonly MessageBinderComparer Default = new MessageBinderComparer();
            public int Compare(IReadOnlyMessageBinder? a, IReadOnlyMessageBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<MessageId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class MessageBinderIdTieBreakComparer : IComparer<IReadOnlyMessageBinder>
        {
            private readonly IComparer<IReadOnlyMessageBinder> _inner;
            public MessageBinderIdTieBreakComparer(IComparer<IReadOnlyMessageBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlyMessageBinder? a, IReadOnlyMessageBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
