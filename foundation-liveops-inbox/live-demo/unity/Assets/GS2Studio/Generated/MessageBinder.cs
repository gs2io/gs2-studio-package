...
{
    /// <summary>
    /// Pure read contract for <see cref="MessageBinder"/>: the read-only
    /// model surface (<see cref="Message"/>) plus the single-fetch
    ...
    /// </summary>
    public interface IReadOnlyMessageBinder : Message
    {
    }

    /// <summary>
    /// Actionable contract for <see cref="MessageBinder"/>: the read contract
    ...
    /// </summary>
    public interface IActionableMessageBinder : IReadOnlyMessageBinder
    {
        Task Read();
    }

    /// <summary>
    /// Owning contract for <see cref="MessageBinder"/>: adds the lifecycle
    /// surface (Subscribe / Invalidate / Dispose) over <see cref="IActionableMessageBinder"/>.
    /// Returned by the binder factory; a fake implementation substitutes the
    /// whole binder.
    /// </summary>
    public interface IMessageBinder : IActionableMessageBinder, IDisposable
    {
    ...
    /// <summary>
    /// Lifecycle-free base for <see cref="MessageBinder"/>.
    /// Owns <c>_model</c> / <c>_gs2</c> / <c>_session</c> and implements the
    /// read-only <see cref="Message"/> contract by forwarding each
    ...
    /// </summary>
    public class ReadOnlyMessageBinder : IReadOnlyMessageBinder
    {
        ...
        private protected readonly MutableMessage _model;
        ...
        /// the read-only <c>Message</c> surface this binder implements.
        /// </summary>
        internal MutableMessage MutableModel => _model;
        /// <inheritdoc cref="Message.Id" />
        public MessageId Id => _model.Id;
        /// <inheritdoc cref="Message.Payload" />
        public string Payload => _model.Payload;
        /// <inheritdoc cref="Message.IsRead" />
        public bool IsRead => _model.IsRead;
        /// <inheritdoc cref="Message.ReceivedAt" />
        public DateTime ReceivedAt => _model.ReceivedAt;
        /// <inheritdoc cref="Message.ExpiresAt" />
        ...
        /// </summary>
        private protected ReadOnlyMessageBinder(MutableMessage model, Gs2Domain gs2, IGameSession session)
        {
        ...
        /// through the read contract <see cref="IReadOnlyMessageBinder"/>) and
    ...
    /// </summary>
    public sealed class MessageBinder : ReadOnlyMessageBinder, IMessageBinder
    {
        private readonly List<Action> _unsubscribers = new List<Action>();
        private bool _disposed;
        internal bool _mounted;

        private readonly Gs2Bind.Gs2Inbox.MessageLoader __userdataInboxMessageLoader;
        ...
        /// </summary>
        internal MessageBinder(
            MutableMessage model,
            Gs2Domain gs2,
            IGameSession session
        ) : base(model, gs2, session)
        {
            __userdataInboxMessageLoader = new Gs2Bind.Gs2Inbox.MessageLoader("Inbox", _model.Id);
        }

        /// <summary>
        /// Creates a Message instance from identity parameters. The
        ...
        /// </summary>
        internal static MutableMessage CreateModel(MessageId id)
        {
            var model = new MutableMessage();
            model.Id = id;
            return model;
        }

        /// <summary>
        /// Creates a MessageBinder and mounts it to GS2 resources.
        /// This is the only external construction path for mount-based usage.
        /// </summary>
        public static async Task<MessageBinder> CreateAsync(
            MessageId id,
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            ...
            var binder = new MessageBinder(model, gs2, session);
        ...
        public async Task MountAsync(CancellationToken cancellationToken = default)
        {
            ...
            var _userdataInboxMessage = await __userdataInboxMessageLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            ApplyUserdataInboxMessage(_model, _userdataInboxMessage);
            _mounted = true;
        }
        ...
        {
            ThrowIfDisposed();
            _unsubscribers.Add(__userdataInboxMessageLoader.Subscribe(
                _gs2,
                _session,
                (_, _, value) =>
                {
                    if (_disposed) return Task.CompletedTask;
                    ApplyUserdataInboxMessage(_model, value);
        ...
        {
            ThrowIfDisposed();
            __userdataInboxMessageLoader.Invalidate(_gs2, _session);
        }
        ...
        /// (callable through <see cref="IReadOnlyMessageBinder"/>) and calls
        ...
        private void EnsureActionContext()
        {
            ...
            if (!_mounted)
            {
                throw new InvalidOperationException(
                    $"{nameof(MessageBinder)} is not mounted. Use CreateAsync or one of the list factories.");
            }
        }
        ...
        {
            EnsureActionContext();
            await new Gs2Bind.Gs2Inbox.MessageLoader("Inbox", _model.Id).Read(_gs2, _session);
        }
        #endregion

        #region Model composition
        /// <summary>
        /// Shared composition for the <c>__userdataInboxMessageLoader</c> source:
        ...
        /// </summary>
        public static void ApplyUserdataInboxMessage(IMutableMessage model, Gs2.Unity.Gs2Inbox.Model.EzMessage? source)
        {
            if (source != null)
            {
                model.ExpiresAt = source.ExpiresAt == 0 ? null : DateTimeOffset.FromUnixTimeMilliseconds(source.ExpiresAt).UtcDateTime;
                model.IsRead = source.IsRead;
                model.Payload = source.Metadata;
                // Skip primary id assignment
                model.ReceivedAt = DateTimeOffset.FromUnixTimeMilliseconds(source.ReceivedAt).UtcDateTime;
            }
            else
            {
                model.ExpiresAt = default;
                model.IsRead = default;
                model.Payload = string.Empty;
                model.ReceivedAt = default;
            }
        }
        #endregion
