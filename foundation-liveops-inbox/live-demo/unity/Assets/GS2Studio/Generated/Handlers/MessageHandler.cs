...
{
    /// <summary>
    /// MonoBehaviour handler for Message. Holds one MessageBinder,
    ...
    /// MessageListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Message/Message Handler")]
    public sealed class MessageHandler : MessageHandlerBase
    {
        ...
        private IMessageBinder? _binder;
        ...
        public override IActionableMessageBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Message"/>, so it is the model.</summary>
        public override Message? Model => _binder;
        ...
        /// to substitute a fake MessageBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a MessageBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new MessageId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned MessageBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a MessageBinderCollection) already
        ...
        /// </summary>
        public void Bind(IMessageBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IMessageBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IMessageBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IMessageBinder binder)
        {
