...
{
    /// <summary>
    /// MonoBehaviour list handler for Message. Owns a
    /// MessageBinderCollection, instantiates a child MessageListItemHandler
    ...
    /// IMessageBinderListSource so items read Binders[Index] through the
    /// shared parent-list surface.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Message/Message List Handler")]
    public sealed class MessageListHandler : MonoBehaviour, IMessageBinderListSource
    {
        [SerializeField] private MessageListItemHandler? _itemPrefab;
        ...
        // Construction seam. Defaults to DefaultMessageBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake collection.
        private IMessageBinderFactory? _binderFactory;

        private IMessageBinderCollection? _collection;
        ...
        private Action<IMessageBinder>? _collectionItemAddedCallback;
        private Action<IMessageBinder>? _collectionItemRemovedCallback;
        ...
        private readonly Dictionary<IActionableMessageBinder, MessageListItemHandler> _handlers =
            new Dictionary<IActionableMessageBinder, MessageListItemHandler>();
        ...
        private IComparer<IReadOnlyMessageBinder>? _comparer;
        ...
        /// itself is the IReadOnlyList&lt;IActionableMessageBinder&gt;.</summary>
        public IReadOnlyList<IActionableMessageBinder> Binders =>
            (IReadOnlyList<IActionableMessageBinder>?)_collection ?? Array.Empty<IActionableMessageBinder>();
        ...
        public event Action<IActionableMessageBinder>? ItemAdded;

        /// <summary>Fires once for each binder removed.</summary>
        public event Action<IActionableMessageBinder>? ItemRemoved;
        ...
        public IComparer<IReadOnlyMessageBinder>? Comparer
        {
            ...
            set
            {
                ...
                // MessageBinderCollection.MessageBinderComparer.
                var applied = value ?? MessageBinderCollection.MessageBinderComparer.Default;
        ...
        /// to substitute a fake MessageBinderCollection; takes precedence over
        /// the default factory. Pass null to revert to the default. Applies to
        /// the next ReloadAsync.
        /// </summary>
        public void SetBinderFactory(IMessageBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IMessageBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultMessageBinderFactory.Instance;
        ...
        /// (the MessageBinderCollection only re-sorts on list-membership
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            var operationGeneration = ++_reloadGeneration;
            IMessageBinderCollection? operationCollection = null;
            try
            {
                Cleanup();
                ...
                await collection.MountFromInboxInboxUserDataAsync(cancellationToken);
                ...
                if (_autoSubscribe)
                {
                    ...
                    collection.SubscribeFromInboxInboxUserData(collectionChanged, collectionFailed);
                }
        ...
        private bool IsCurrentOperation(long operationGeneration, IMessageBinderCollection? operationCollection)
            => !_isDestroyed
        ...
        private void AttachCollectionCallbacks(
            ...
            IMessageBinderCollection collection)
        {
            Action<IMessageBinder> itemAdded = binder =>
                OnItemAdded(operationGeneration, collection, binder);
            Action<IMessageBinder> itemRemoved = binder =>
        ...
        private void OnCollectionChanged(long operationGeneration, IMessageBinderCollection collection)
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
        ...
        private int IndexOf(IActionableMessageBinder binder)
        {
            var binders = Binders;
        ...
        {
            if (!IsCurrentOperation(operationGeneration, collection)) return;
            // The collection is the IReadOnlyList<IActionableMessageBinder>; index it
        ...
        private void Cleanup()
        {
            ...
            var handlers = new List<MessageListItemHandler>(_handlers.Values);
            _collection = null;
