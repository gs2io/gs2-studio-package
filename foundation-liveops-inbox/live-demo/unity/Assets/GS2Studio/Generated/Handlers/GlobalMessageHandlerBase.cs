...
namespace GS2Studio.Generated.GlobalMessage
{
    ...
    /// GlobalMessageBinder: the standalone/list GlobalMessageHandler and the
    /// slot-bound GlobalMessageListItemHandler. Concrete handlers supply the
    ...
    /// </summary>
    public abstract class GlobalMessageHandlerBase : MonoBehaviour
    {
        /// <summary>The bound binder (non-owning actionable view), or null when none is attached.</summary>
        public abstract IActionableGlobalMessageBinder? Binder { get; }

        /// <summary>The bound model, or null when no binder is attached.</summary>
        public abstract GlobalMessage? Model { get; }
        ...
        public event Action<GlobalMessage>? Updated;

        /// <summary>Fires once each time a binder is attached (non-owning actionable view).</summary>
        public event Action<IActionableGlobalMessageBinder>? Bound;
        ...
        protected void RaiseUpdated(GlobalMessage model) => Updated?.Invoke(model);
        protected void RaiseBound(IActionableGlobalMessageBinder binder) => Bound?.Invoke(binder);
