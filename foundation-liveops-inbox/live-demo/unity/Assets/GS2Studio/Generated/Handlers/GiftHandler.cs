...
{
    /// <summary>
    /// MonoBehaviour handler for Gift. Holds one GiftBinder,
    ...
    /// GiftListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Gift/Gift Handler")]
    public sealed class GiftHandler : GiftHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultGiftBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IGiftBinderFactory? _binderFactory;

        private IGiftBinder? _binder;
        ...
        public override IActionableGiftBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Gift"/>, so it is the model.</summary>
        public override Gift? Model => _binder;
        ...
        /// to substitute a fake GiftBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IGiftBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IGiftBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultGiftBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a GiftBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new GiftId("gift"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned GiftBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a GiftBinderCollection) already
        ...
        /// </summary>
        public void Bind(IGiftBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IGiftBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IGiftBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IGiftBinder binder)
        {
