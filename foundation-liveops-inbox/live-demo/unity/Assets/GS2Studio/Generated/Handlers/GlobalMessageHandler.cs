...
{
    /// <summary>
    /// MonoBehaviour handler for GlobalMessage. Holds one GlobalMessageBinder,
    ...
    /// GlobalMessageListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/GlobalMessage/GlobalMessage Handler")]
    public sealed class GlobalMessageHandler : GlobalMessageHandlerBase
    {
        ...
        private IGlobalMessageBinder? _binder;
        ...
        public override IActionableGlobalMessageBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="GlobalMessage"/>, so it is the model.</summary>
        public override GlobalMessage? Model => _binder;
        ...
        /// to substitute a fake GlobalMessageBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a GlobalMessageBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new GlobalMessageId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned GlobalMessageBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a GlobalMessageBinderCollection) already
        ...
        /// </summary>
        public void Bind(IGlobalMessageBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IGlobalMessageBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IGlobalMessageBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IGlobalMessageBinder binder)
        {
