...
using GS2Studio.Generated.Gift;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.Gift.UI
{
    /// <summary>
    /// UI button bound to <c>Deliver</c> on the sibling
    /// <c>GiftHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>GiftHandlerBase.Binder.Deliver</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>GiftHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Gift/ButtonAction/DeliverButton")]
    public sealed class GiftDeliverButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private GiftHandlerBase? _handler;
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<GiftHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(GiftDeliverButton)} on '{name}': no GiftHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"GiftDeliverButton: Deliver failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"GiftDeliverButton: Deliver failed: {ex}");
                return;
            }
            finally
            {
