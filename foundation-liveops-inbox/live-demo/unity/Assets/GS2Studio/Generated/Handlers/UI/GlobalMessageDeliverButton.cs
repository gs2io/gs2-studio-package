...
using GS2Studio.Generated.GlobalMessage;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.GlobalMessage.UI
{
    /// <summary>
    /// UI button bound to <c>Deliver</c> on the sibling
    /// <c>GlobalMessageHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>GlobalMessageHandlerBase.Binder.Deliver</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>GlobalMessageHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/GlobalMessage/ButtonAction/DeliverButton")]
    public sealed class GlobalMessageDeliverButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private GlobalMessageHandlerBase? _handler;
        ...
        /// in that window is dropped, not queued: a second <c>Deliver</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<GlobalMessageHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(GlobalMessageDeliverButton)} on '{name}': no GlobalMessageHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"GlobalMessageDeliverButton: Deliver failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"GlobalMessageDeliverButton: Deliver failed: {ex}");
                return;
            }
            finally
            {
