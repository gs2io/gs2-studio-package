...
namespace GS2Studio.Generated.Message.UI
{
    ...
    /// based on the condition <c>not(truthy(isRead))</c> evaluated
    /// against the sibling <c>MessageHandler.Model</c>:
    /// <list type="bullet">
    ...
    /// <c>MessageHandler</c> or <c>MessageListItemHandler</c>;
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Message/ActiveToggle/UnreadActiveToggle")]
    public sealed class MessageUnreadActiveToggle : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(MessageUnreadActiveToggle)} on '{name}': no MessageHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            bool result = !(((object?)model.IsRead) switch { null => false, bool __b => __b, string __s => !string.IsNullOrEmpty(__s), int __i => __i != 0, long __l => __l != 0L, float __f => __f != 0f, double __d => __d != 0d, _ => true });
