...
{
    /// <summary>
    /// UI button bound to <c>Read</c> on the sibling
    /// <c>MessageHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>MessageHandlerBase.Binder.Read</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>MessageHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Message/ButtonAction/OpenButton")]
    public sealed class MessageOpenButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Read</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(MessageOpenButton)} on '{name}': no MessageHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"MessageOpenButton: Read failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"MessageOpenButton: Read failed: {ex}");
                return;
            }
            finally
            {
