...
namespace GS2Studio.Generated.CharacterFormation.UI
{
    /// <summary>
    /// Drives <c>UnityEngine.UI.Selectable.interactable</c> on two Selectable
    /// groups based on the condition <c>lt(currentSaveArea, maximumSaveArea)</c>
    /// evaluated against the sibling <c>CharacterFormationHandler.Model</c>:
    /// <list type="bullet">
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormation/Interactable/ExpandInteractable")]
    public sealed class CharacterFormationExpandInteractable : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private CharacterFormationHandlerBase? _handler;
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterFormationExpandInteractable)} on '{name}': no CharacterFormationHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<CharacterFormationHandlerBase>();
        }

        private void OnUpdated(CharacterFormation model)
        {
            bool result = (System.Convert.ToInt64(model.CurrentSaveArea, System.Globalization.CultureInfo.InvariantCulture) < System.Convert.ToInt64(model.MaximumSaveArea, System.Globalization.CultureInfo.InvariantCulture));
