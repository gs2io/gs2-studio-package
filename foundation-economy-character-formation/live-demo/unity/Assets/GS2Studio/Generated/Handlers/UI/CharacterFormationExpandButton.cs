...
namespace GS2Studio.Generated.CharacterFormation.UI
{
    ...
    /// <c>CharacterFormationHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>CharacterFormationHandlerBase.Binder.Expand</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>CharacterFormationHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormation/ButtonAction/ExpandButton")]
    public sealed class CharacterFormationExpandButton : MonoBehaviour
    {
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterFormationExpandButton)} on '{name}': no CharacterFormationHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"CharacterFormationExpandButton: Expand failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"CharacterFormationExpandButton: Expand failed: {ex}");
                return;
            }
            finally
            {
