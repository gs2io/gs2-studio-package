...
namespace GS2Studio.Generated.CharacterFormation.UI
{
    ...
    /// <c>{currentSaveArea} of {maximumSaveArea} parties</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>CharacterFormation</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormation/TemplateLabel/SaveAreaLabel")]
    public sealed class CharacterFormationSaveAreaLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterFormationSaveAreaLabel)} on '{name}': no CharacterFormationHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.CurrentSaveArea} of {model.MaximumSaveArea} parties");
        }
    }
}
