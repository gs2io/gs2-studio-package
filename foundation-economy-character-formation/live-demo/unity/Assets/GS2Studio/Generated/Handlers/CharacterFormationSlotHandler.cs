...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterFormationSlot. Holds one CharacterFormationSlotBinder,
    ...
    /// CharacterFormationSlotListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormationSlot/CharacterFormationSlot Handler")]
    public sealed class CharacterFormationSlotHandler : CharacterFormationSlotHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterFormationSlotBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterFormationSlotBinderFactory? _binderFactory;

        private ICharacterFormationSlotBinder? _binder;
        ...
        public override IActionableCharacterFormationSlotBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterFormationSlot"/>, so it is the model.</summary>
        public override CharacterFormationSlot? Model => _binder;
        ...
        /// to substitute a fake CharacterFormationSlotBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterFormationSlotBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterFormationSlotBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterFormationSlotBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CharacterFormationSlotBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterFormationSlotId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterFormationSlotBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterFormationSlotBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterFormationSlotBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterFormationSlotBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterFormationSlotBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterFormationSlotBinder binder)
        {
