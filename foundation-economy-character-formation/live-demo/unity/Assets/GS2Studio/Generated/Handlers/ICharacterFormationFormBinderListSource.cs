// <auto-generated>
//     This code was generated by CSharpHandlerGeneratorService.
//     Do not modify this file directly.
// </auto-generated>

#nullable enable

using System;
using System.Collections.Generic;
using Gs2.Unity.Gs2Formation.Model;
using Gs2Bind.Gs2Formation;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.CharacterFormationForm
{
    /// <summary>
    /// Read surface shared by CharacterFormationFormListHandler and the ref-scoped
    /// CharacterFormationFormListBy*Handler variants. CharacterFormationFormListItemHandler
    /// resolves its parent list through this interface, so one item prefab
    /// serves every list flavor.
    /// </summary>
    public interface ICharacterFormationFormBinderListSource
    {
        /// <summary>Stable index/enumeration view of the current non-owning binder set; Model.Id is not a unique lookup key.</summary>
        IReadOnlyList<IActionableCharacterFormationFormBinder> Binders { get; }

        /// <summary>Fires after reload and on each subsequent membership/order change.</summary>
        event Action? ListChanged;
    }
}
