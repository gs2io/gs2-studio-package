...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterFormation. Holds one CharacterFormationBinder,
    ...
    /// CharacterFormationListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormation/CharacterFormation Handler")]
    public sealed class CharacterFormationHandler : CharacterFormationHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterFormationBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterFormationBinderFactory? _binderFactory;

        private ICharacterFormationBinder? _binder;
        ...
        public override IActionableCharacterFormationBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterFormation"/>, so it is the model.</summary>
        public override CharacterFormation? Model => _binder;
        ...
        /// to substitute a fake CharacterFormationBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterFormationBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterFormationBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterFormationBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CharacterFormationBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterFormationId("characterformation"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterFormationBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterFormationBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterFormationBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterFormationBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterFormationBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterFormationBinder binder)
        {
