...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterFormationForm. Holds one CharacterFormationFormBinder,
    ...
    /// CharacterFormationFormListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterFormationForm/CharacterFormationForm Handler")]
    public sealed class CharacterFormationFormHandler : CharacterFormationFormHandlerBase
    {
        ...
        private ICharacterFormationFormBinder? _binder;
        ...
        public override IActionableCharacterFormationFormBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterFormationForm"/>, so it is the model.</summary>
        public override CharacterFormationForm? Model => _binder;
        ...
        /// to substitute a fake CharacterFormationFormBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a CharacterFormationFormBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterFormationFormId(_index.ToString()), _index, gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterFormationFormBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterFormationFormBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterFormationFormBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterFormationFormBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterFormationFormBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterFormationFormBinder binder)
        {
