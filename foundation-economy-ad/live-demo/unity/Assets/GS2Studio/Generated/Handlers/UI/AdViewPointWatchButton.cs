...
{
    /// <summary>
    /// UI button bound to <c>Watch</c> on the sibling
    /// <c>AdViewPointHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>AdViewPointHandlerBase.Binder.Watch</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>AdViewPointHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/AdViewPoint/ButtonAction/WatchButton")]
    public sealed class AdViewPointWatchButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Watch</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(AdViewPointWatchButton)} on '{name}': no AdViewPointHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"AdViewPointWatchButton: Watch failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"AdViewPointWatchButton: Watch failed: {ex}");
                return;
            }
            finally
            {
