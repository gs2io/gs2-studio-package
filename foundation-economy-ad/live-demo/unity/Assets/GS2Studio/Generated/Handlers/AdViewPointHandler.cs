...
{
    /// <summary>
    /// MonoBehaviour handler for AdViewPoint. Holds one AdViewPointBinder,
    ...
    /// AdViewPointListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/AdViewPoint/AdViewPoint Handler")]
    public sealed class AdViewPointHandler : AdViewPointHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultAdViewPointBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private IAdViewPointBinderFactory? _binderFactory;

        private IAdViewPointBinder? _binder;
        ...
        public override IActionableAdViewPointBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="AdViewPoint"/>, so it is the model.</summary>
        public override AdViewPoint? Model => _binder;
        ...
        /// to substitute a fake AdViewPointBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(IAdViewPointBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private IAdViewPointBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultAdViewPointBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a AdViewPointBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new AdViewPointId("adviewpoint"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned AdViewPointBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a AdViewPointBinderCollection) already
        ...
        /// </summary>
        public void Bind(IAdViewPointBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IAdViewPointBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IAdViewPointBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IAdViewPointBinder binder)
        {
