...
namespace GS2Studio.Generated.Character.UI
{
    ...
    /// groups based on the condition <c>not(truthy(acquired))</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Character/Interactable/UnregisteredInteractable")]
    public sealed class CharacterUnregisteredInteractable : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterUnregisteredInteractable)} on '{name}': no CharacterHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            bool result = !(((object?)model.Acquired) switch { null => false, bool __b => __b, string __s => !string.IsNullOrEmpty(__s), int __i => __i != 0, long __l => __l != 0L, float __f => __f != 0f, double __d => __d != 0d, _ => true });
