...
namespace GS2Studio.Generated.Character
{
    ...
    public interface ICharacterBinderCollection : IReadOnlyCharacterBinderCollection, IDisposable
    {
        ...
        void Invalidate();
        Task MountFromExchangeCharacterDictionaryMarkMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromExchangeCharacterDictionaryMarkMasterData(Action? onChange = null, Action<Exception>? onError = null);
        ...
        Task MountFromDictionaryCharacterDictionaryUserDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromDictionaryCharacterDictionaryUserData(Action? onChange = null, Action<Exception>? onError = null);
    }
    ...
    public sealed class CharacterBinderCollection : ReadOnlyCharacterBinderCollection, ICharacterBinderCollection
    {
        ...
        {
            ThrowIfDisposed();
            new RateModelArrayLoader("CharacterDictionaryMark").Invalidate(_gs2, _session);
        ...
        /// <summary>One-shot Create + MountFromExchangeCharacterDictionaryMarkMasterDataAsync (no subscription).</summary>
        public static async Task<CharacterBinderCollection> CreateFromExchangeCharacterDictionaryMarkMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            string propertyId,
            CancellationToken cancellationToken = default)
        {
            ...
            await coll.MountFromExchangeCharacterDictionaryMarkMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromExchangeCharacterDictionaryMarkMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
            var arrayLoader = new RateModelArrayLoader("CharacterDictionaryMark");
            var items = await arrayLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            await ReconcileFromExchangeCharacterDictionaryMarkMasterItems(items, attachChildSubscribe: false, cancellationToken);
            _mounted = true;
        }

        public void SubscribeFromExchangeCharacterDictionaryMarkMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromExchangeCharacterDictionaryMarkMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromExchangeCharacterDictionaryMarkMasterItems(IList<EzRateModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            var seen = new HashSet<string>();
            foreach (var item in items)
            {
                if (_disposed) return;
                var rowKey = ExtractExchangeCharacterDictionaryMarkMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyExchangeCharacterDictionaryMarkMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromExchangeCharacterDictionaryMarkMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    {
                        ApplyExchangeCharacterDictionaryMarkMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
        ...
        private async Task<CharacterBinder> BuildBinderFromExchangeCharacterDictionaryMarkMasterItem(EzRateModel item, CancellationToken cancellationToken)
        {
            ...
            ApplyExchangeCharacterDictionaryMarkMasterItemTo(model, item);
            var binder = new CharacterBinder(model, _gs2, _session);
            await binder.MountAsync(cancellationToken);
            return binder;
        }

        private static void ApplyExchangeCharacterDictionaryMarkMasterItemTo(MutableCharacter model, EzRateModel item)
        {
        ...
        private CharacterId ExtractExchangeCharacterDictionaryMarkMasterIdentity(EzRateModel item)
        {
            return (string.IsNullOrEmpty(item.Name) ? default(CharacterId) : new CharacterId(item.Name));
        }

        private string? ExtractExchangeCharacterDictionaryMarkMasterRowKey(EzRateModel item)
        {
            var id = ExtractExchangeCharacterDictionaryMarkMasterIdentity(item);
            if (EqualityComparer<CharacterId>.Default.Equals(id, default)) return null;
            if (string.IsNullOrEmpty(item.Name)) return null;
            return $"{item.Name}";
        }
        /// <summary>One-shot Create + MountFromDictionaryCharacterDictionaryMasterDataAsync (no subscription).</summary>
        public static async Task<CharacterBinderCollection> CreateFromDictionaryCharacterDictionaryMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            string propertyId,
            CancellationToken cancellationToken = default)
        {
            var coll = new CharacterBinderCollection(gs2, session, propertyId);
            await coll.MountFromDictionaryCharacterDictionaryMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromDictionaryCharacterDictionaryMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
        ...
        public void SubscribeFromDictionaryCharacterDictionaryMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            if (_subscriptionActive) throw new InvalidOperationException("Already subscribed");
            _subscriptionActive = true;
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromDictionaryCharacterDictionaryMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
                        if (onError != null) onError(ex); else UnityEngine.Debug.LogException(ex);
        ...
        private async Task ReconcileFromDictionaryCharacterDictionaryMasterItems(IList<EzEntryModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            ...
            {
                if (_disposed) return;
                var rowKey = ExtractDictionaryCharacterDictionaryMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyDictionaryCharacterDictionaryMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromDictionaryCharacterDictionaryMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    if (_bindersByRowKey.TryGetValue(rowKey, out var raced))
                    {
                        ApplyDictionaryCharacterDictionaryMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
                    if (attachChildSubscribe) binder.Subscribe(_onChange);
        ...
        /// <summary>One-shot Create + MountFromDictionaryCharacterDictionaryUserDataAsync (no subscription).</summary>
        public static async Task<CharacterBinderCollection> CreateFromDictionaryCharacterDictionaryUserDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            string propertyId,
            CancellationToken cancellationToken = default)
        {
            var coll = new CharacterBinderCollection(gs2, session, propertyId);
            await coll.MountFromDictionaryCharacterDictionaryUserDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromDictionaryCharacterDictionaryUserDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
        ...
        public void SubscribeFromDictionaryCharacterDictionaryUserData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            if (_subscriptionActive) throw new InvalidOperationException("Already subscribed");
            _subscriptionActive = true;
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromDictionaryCharacterDictionaryUserItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
                        if (onError != null) onError(ex); else UnityEngine.Debug.LogException(ex);
        ...
        private async Task ReconcileFromDictionaryCharacterDictionaryUserItems(IList<EzEntry> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            ...
            {
                if (_disposed) return;
                var rowKey = ExtractDictionaryCharacterDictionaryUserRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyDictionaryCharacterDictionaryUserItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromDictionaryCharacterDictionaryUserItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    if (_bindersByRowKey.TryGetValue(rowKey, out var raced))
                    {
                        ApplyDictionaryCharacterDictionaryUserItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
                    if (attachChildSubscribe) binder.Subscribe(_onChange);
