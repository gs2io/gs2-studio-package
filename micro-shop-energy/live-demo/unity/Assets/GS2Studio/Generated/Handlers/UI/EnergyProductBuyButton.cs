...
using GS2Studio.Generated.EnergyProduct;
using GS2Studio.Generated.Runtime;

namespace GS2Studio.Generated.EnergyProduct.UI
{
    /// <summary>
    /// UI button bound to <c>Buy</c> on the sibling
    /// <c>EnergyProductHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EnergyProductHandlerBase.Binder.Buy</c> and forwards the
    /// authored argument list. Add this component alongside (or under) a
    /// <c>EnergyProductHandlerBase</c>; the handler is resolved automatically via
    /// <c>GetComponentInParent&lt;&gt;</c> when no Inspector reference is set.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EnergyProduct/ButtonAction/BuyButton")]
    public sealed class EnergyProductBuyButton : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private EnergyProductHandlerBase? _handler;
        ...
        /// in that window is dropped, not queued: a second <c>Buy</c>
        ...
        {
            if (_handler == null) _handler = GetComponentInParent<EnergyProductHandlerBase>();
        }
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EnergyProductBuyButton)} on '{name}': no EnergyProductHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"EnergyProductBuyButton: Buy failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EnergyProductBuyButton: Buy failed: {ex}");
                return;
            }
            finally
            {
