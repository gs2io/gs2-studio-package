...
{
    /// <summary>
    /// UI button bound to <c>Recover</c> on the sibling
    /// <c>EnergyHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>EnergyHandlerBase.Binder.Recover</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Energy/ButtonAction/RecoverButton")]
    public sealed class EnergyRecoverButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Recover</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EnergyRecoverButton)} on '{name}': no EnergyHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"EnergyRecoverButton: Recover failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"EnergyRecoverButton: Recover failed: {ex}");
                return;
            }
            finally
            {
