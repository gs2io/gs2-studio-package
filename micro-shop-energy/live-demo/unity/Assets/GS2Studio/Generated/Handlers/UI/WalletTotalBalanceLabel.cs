...
{
    /// <summary>
    /// UI label bound to <c>Wallet.total</c>.
    ...
    /// supplied.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Wallet/Label/TotalBalanceLabel")]
    public sealed class WalletTotalBalanceLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(WalletTotalBalanceLabel)} on '{name}': no WalletHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke(System.Convert.ToString(model.Total) ?? "");
        }
    }
}
