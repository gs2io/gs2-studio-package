...
{
    /// <summary>
    /// MonoBehaviour handler for Energy. Holds one EnergyBinder,
    ...
    /// EnergyListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Energy/Energy Handler")]
    public sealed class EnergyHandler : EnergyHandlerBase
    {
        ...
        /// binder implements <see cref="Energy"/>, so it is the model.</summary>
        public override Energy? Model => _binder;
        ...
        /// to substitute a fake EnergyBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a EnergyBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EnergyId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EnergyBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EnergyBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEnergyBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEnergyBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEnergyBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEnergyBinder binder)
        {
