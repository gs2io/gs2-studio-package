...
{
    /// <summary>
    /// MonoBehaviour handler for EnergyProduct. Holds one EnergyProductBinder,
    ...
    /// EnergyProductListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/EnergyProduct/EnergyProduct Handler")]
    public sealed class EnergyProductHandler : EnergyProductHandlerBase
    {
        ...
        private IEnergyProductBinder? _binder;
        ...
        public override IActionableEnergyProductBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="EnergyProduct"/>, so it is the model.</summary>
        public override EnergyProduct? Model => _binder;
        ...
        /// to substitute a fake EnergyProductBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a EnergyProductBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new EnergyProductId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned EnergyProductBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a EnergyProductBinderCollection) already
        ...
        /// </summary>
        public void Bind(IEnergyProductBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IEnergyProductBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IEnergyProductBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IEnergyProductBinder binder)
        {
