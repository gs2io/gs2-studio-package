...
{
    /// <summary>
    /// MonoBehaviour handler for Schedule. Holds one ScheduleBinder,
    ...
    /// ScheduleListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Schedule/Schedule Handler")]
    public sealed class ScheduleHandler : ScheduleHandlerBase
    {
        ...
        private IScheduleBinder? _binder;
        ...
        /// binder implements <see cref="Schedule"/>, so it is the model.</summary>
        public override Schedule? Model => _binder;
        ...
        /// to substitute a fake ScheduleBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a ScheduleBinder from the current identity keys, mounts it,
        ...
        /// <summary>
        /// Attaches an externally-owned ScheduleBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a ScheduleBinderCollection) already
        ...
        /// </summary>
        public void Bind(IScheduleBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IScheduleBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IScheduleBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IScheduleBinder binder)
        {
