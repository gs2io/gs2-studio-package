...
{
    /// <summary>
    /// MonoBehaviour handler for SkillNode. Holds one SkillNodeBinder,
    ...
    /// SkillNodeListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/SkillNode/SkillNode Handler")]
    public sealed class SkillNodeHandler : SkillNodeHandlerBase
    {
        ...
        private ISkillNodeBinder? _binder;
        ...
        public override IActionableSkillNodeBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="SkillNode"/>, so it is the model.</summary>
        public override SkillNode? Model => _binder;
        ...
        /// to substitute a fake SkillNodeBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string owner)
        {
        ...
        /// <summary>
        /// Creates a SkillNodeBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new SkillNodeId(_id ?? string.Empty), _owner, gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned SkillNodeBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a SkillNodeBinderCollection) already
        ...
        /// </summary>
        public void Bind(ISkillNodeBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ISkillNodeBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ISkillNodeBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ISkillNodeBinder binder)
        {
