...
namespace GS2Studio.Generated.SkillNode.UI
{
    ...
    /// <c>A reset returns {rate} of what it cost.</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>SkillNode</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/SkillNode/TemplateLabel/UnlockedLabel")]
    public sealed class SkillNodeUnlockedLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(SkillNodeUnlockedLabel)} on '{name}': no SkillNodeHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"A reset returns {model.RestrainReturnRate} of what it cost.");
        }
    }
}
