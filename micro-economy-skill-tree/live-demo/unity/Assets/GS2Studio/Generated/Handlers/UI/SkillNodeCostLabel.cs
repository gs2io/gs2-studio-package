...
namespace GS2Studio.Generated.SkillNode.UI
{
    ...
    /// <c>{cost} free currency</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/SkillNode/TemplateLabel/CostLabel")]
    public sealed class SkillNodeCostLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(SkillNodeCostLabel)} on '{name}': no SkillNodeHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.Cost} free currency");
        }
    }
}
