...
namespace GS2Studio.Generated.SkillNode.UI
{
    /// <summary>
    /// Drives <c>GameObject.SetActive(bool)</c> on two GameObject groups
    /// based on the condition <c>not(truthy(released))</c> evaluated
    /// against the sibling <c>SkillNodeHandler.Model</c>:
    /// <list type="bullet">
    ...
    /// <c>SkillNodeHandler</c> or <c>SkillNodeListItemHandler</c>;
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/SkillNode/ActiveToggle/UnreleasedActiveToggle")]
    public sealed class SkillNodeUnreleasedActiveToggle : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(SkillNodeUnreleasedActiveToggle)} on '{name}': no SkillNodeHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        private void OnUpdated(SkillNode model)
        {
            bool result = !(((object?)model.Released) switch { null => false, bool __b => __b, string __s => !string.IsNullOrEmpty(__s), int __i => __i != 0, long __l => __l != 0L, float __f => __f != 0f, double __d => __d != 0d, _ => true });
