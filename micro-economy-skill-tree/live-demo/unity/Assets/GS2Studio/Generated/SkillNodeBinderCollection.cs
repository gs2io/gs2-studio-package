...
{
    /// <summary>
    /// Public read contract for the SkillNode binder collection: the
    ...
    /// </summary>
    public interface IReadOnlySkillNodeBinderCollection : IReadOnlyList<IActionableSkillNodeBinder>
    {
    }

    /// <summary>
    /// Owning contract for the SkillNode binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlySkillNodeBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface ISkillNodeBinderCollection : IReadOnlySkillNodeBinderCollection, IDisposable
    {
        IComparer<IReadOnlySkillNodeBinder> Comparer { get; set; }
        event Action<ISkillNodeBinder> ItemAdded;
        event Action<ISkillNodeBinder> ItemRemoved;
        void Invalidate();
        Task MountFromSkillTreeSkillTreeMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromSkillTreeSkillTreeMasterData(Action? onChange = null, Action<Exception>? onError = null);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="SkillNodeBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableSkillNodeBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlySkillNodeBinderCollection : IReadOnlySkillNodeBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly SkillNodeBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlySkillNodeBinder>? _predicate;
        private readonly List<IActionableSkillNodeBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableSkillNodeBinder> EmptyList = new List<IActionableSkillNodeBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlySkillNodeBinderCollection Empty { get; } = new ReadOnlySkillNodeBinderCollection();
        ...
        /// </summary>
        protected ReadOnlySkillNodeBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlySkillNodeBinderCollection(
            SkillNodeBinderCollection rootSource,
            Predicate<IReadOnlySkillNodeBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableSkillNodeBinder>();
        ...
        public virtual IActionableSkillNodeBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableSkillNodeBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual SkillNodeBinderCollection? GetRootSource() => _rootSource;
    ...
    /// <summary>
    /// Collection binder for SkillNode.
    /// Wraps a list of SkillNodeBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class SkillNodeBinderCollection : ReadOnlySkillNodeBinderCollection, ISkillNodeBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;
        private readonly string _owner;
        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableSkillNodeBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning SkillNodeBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<SkillNodeBinder> _binders = new List<SkillNodeBinder>();
        ...
        private readonly Dictionary<string, SkillNodeBinder> _bindersByRowKey = new Dictionary<string, SkillNodeBinder>();
        ...
        private readonly List<WeakReference<ReadOnlySkillNodeBinderCollection>> _derivedViews = new List<WeakReference<ReadOnlySkillNodeBinderCollection>>();
        ...
        private IComparer<IReadOnlySkillNodeBinder> _configuredComparer = SkillNodeBinderComparer.Default;
        private IComparer<IReadOnlySkillNodeBinder> _effectiveComparer =
            new SkillNodeBinderIdTieBreakComparer(SkillNodeBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlySkillNodeBinder so
        /// it never exposes the owning binder.
        /// </summary>
        public IComparer<IReadOnlySkillNodeBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new SkillNodeBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableSkillNodeBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableSkillNodeBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableSkillNodeBinder contract.
        /// </summary>
        public override IEnumerator<IActionableSkillNodeBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override SkillNodeBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (ISkillNodeBinder).
        // Public so a fake ISkillNodeBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableSkillNodeBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<ISkillNodeBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<ISkillNodeBinder>? ItemRemoved;

        public SkillNodeBinderCollection(
            Gs2Domain gs2,
        ...
        {
            ThrowIfDisposed();
            new Gs2Bind.Gs2SkillTree.NodeModelArrayLoader("SkillTree").Invalidate(_gs2, _session);
        }
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlySkillNodeBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlySkillNodeBinderCollection>(view));
        }
        ...
        /// <summary>One-shot Create + MountFromSkillTreeSkillTreeMasterDataAsync (no subscription).</summary>
        public static async Task<SkillNodeBinderCollection> CreateFromSkillTreeSkillTreeMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            string owner,
            CancellationToken cancellationToken = default)
        {
            var coll = new SkillNodeBinderCollection(gs2, session, owner);
            await coll.MountFromSkillTreeSkillTreeMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromSkillTreeSkillTreeMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            cancellationToken.ThrowIfCancellationRequested();
            var arrayLoader = new Gs2Bind.Gs2SkillTree.NodeModelArrayLoader("SkillTree");
            var items = await arrayLoader.Load(_gs2, _session);
            cancellationToken.ThrowIfCancellationRequested();
            await ReconcileFromSkillTreeSkillTreeMasterItems(items, attachChildSubscribe: false, cancellationToken);
            _mounted = true;
        }

        public void SubscribeFromSkillTreeSkillTreeMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromSkillTreeSkillTreeMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromSkillTreeSkillTreeMasterItems(IList<Gs2.Unity.Gs2SkillTree.Model.EzNodeModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            var seen = new HashSet<string>();
            foreach (var item in items)
            {
                if (_disposed) return;
                var rowKey = ExtractSkillTreeSkillTreeMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplySkillTreeSkillTreeMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromSkillTreeSkillTreeMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    {
                        ApplySkillTreeSkillTreeMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
        ...
        private async Task<SkillNodeBinder> BuildBinderFromSkillTreeSkillTreeMasterItem(Gs2.Unity.Gs2SkillTree.Model.EzNodeModel item, CancellationToken cancellationToken)
        {
            var model = SkillNodeBinder.CreateModel((string.IsNullOrEmpty(item.Name) ? default(SkillNodeId) : new SkillNodeId(item.Name)), _owner);
            ApplySkillTreeSkillTreeMasterItemTo(model, item);
            var binder = new SkillNodeBinder(model, _gs2, _session);
            await binder.MountAsync(cancellationToken);
            return binder;
        }

        private static void ApplySkillTreeSkillTreeMasterItemTo(MutableSkillNode model, Gs2.Unity.Gs2SkillTree.Model.EzNodeModel item)
        {
        ...
        private SkillNodeId ExtractSkillTreeSkillTreeMasterIdentity(Gs2.Unity.Gs2SkillTree.Model.EzNodeModel item)
        {
            return (string.IsNullOrEmpty(item.Name) ? default(SkillNodeId) : new SkillNodeId(item.Name));
        }

        private string? ExtractSkillTreeSkillTreeMasterRowKey(Gs2.Unity.Gs2SkillTree.Model.EzNodeModel item)
        {
            var id = ExtractSkillTreeSkillTreeMasterIdentity(item);
            if (EqualityComparer<SkillNodeId>.Default.Equals(id, default)) return null;
            if (string.IsNullOrEmpty(item.Name)) return null;
            return $"{item.Name}";
        }

        internal sealed class SkillNodeBinderComparer : IComparer<IReadOnlySkillNodeBinder>
        {
            public static readonly SkillNodeBinderComparer Default = new SkillNodeBinderComparer();
            public int Compare(IReadOnlySkillNodeBinder? a, IReadOnlySkillNodeBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<SkillNodeId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class SkillNodeBinderIdTieBreakComparer : IComparer<IReadOnlySkillNodeBinder>
        {
            private readonly IComparer<IReadOnlySkillNodeBinder> _inner;
            public SkillNodeBinderIdTieBreakComparer(IComparer<IReadOnlySkillNodeBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlySkillNodeBinder? a, IReadOnlySkillNodeBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
