...
{
    /// <summary>
    /// MonoBehaviour handler for Item. Holds one ItemBinder,
    ...
    /// ItemListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Item/Item Handler")]
    public sealed class ItemHandler : ItemHandlerBase
    {
        ...
        public override IActionableItemBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="Item"/>, so it is the model.</summary>
        public override Item? Model => _binder;
        ...
        /// to substitute a fake ItemBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a ItemBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new ItemId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned ItemBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a ItemBinderCollection) already
        ...
        /// </summary>
        public void Bind(IItemBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IItemBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IItemBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IItemBinder binder)
        {
