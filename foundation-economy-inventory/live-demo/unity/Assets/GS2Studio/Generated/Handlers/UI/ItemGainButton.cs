...
{
    /// <summary>
    /// UI button bound to <c>Gain</c> on the sibling
    /// <c>ItemHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>ItemHandlerBase.Binder.Gain</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Item/ButtonAction/GainButton")]
    public sealed class ItemGainButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Gain</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(ItemGainButton)} on '{name}': no ItemHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"ItemGainButton: Gain failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"ItemGainButton: Gain failed: {ex}");
                return;
            }
            finally
            {
