...
{
    /// <summary>
    /// UI value bound to <c>Item.count</c>.
    /// Subscribes to the sibling <c>ItemHandler.Updated</c> event
    /// and publishes the resolved <c>long</c> value through
    ...
    /// code to any <c>UnityEvent&lt;long&gt;</c> listener. Add this
    /// component alongside (or under) a <c>ItemHandler</c> or
    /// <c>ItemListItemHandler</c>; the handler is resolved
    ...
    /// persists it.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Item/Value/CountValue")]
    public sealed class ItemCountValue : MonoBehaviour
    {
        [Gs2AutoResolvedHandler]
        [SerializeField] private ItemHandlerBase? _handler;
        [SerializeField] private UnityEvent<long> _onUpdate = new UnityEvent<long>();
        [SerializeField] private UnityEvent<long> _onSelect = new UnityEvent<long>();

        public UnityEvent<long> OnUpdate => _onUpdate;
        public UnityEvent<long> OnSelect => _onSelect;
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(ItemCountValue)} on '{name}': no ItemHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
