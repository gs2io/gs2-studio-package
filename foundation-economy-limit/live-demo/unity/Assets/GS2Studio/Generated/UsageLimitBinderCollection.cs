...
{
    /// <summary>
    /// Public read contract for the UsageLimit binder collection: the
    ...
    /// </summary>
    public interface IReadOnlyUsageLimitBinderCollection : IReadOnlyList<IActionableUsageLimitBinder>
    {
    }

    /// <summary>
    /// Owning contract for the UsageLimit binder collection: adds the
    /// lifecycle + mount/subscribe surface and the owning-binder wiring events
    /// over <see cref="IReadOnlyUsageLimitBinderCollection"/>. Returned by the binder
    /// factory; a fake implementation substitutes the whole collection.
    /// </summary>
    public interface IUsageLimitBinderCollection : IReadOnlyUsageLimitBinderCollection, IDisposable
    {
        IComparer<IReadOnlyUsageLimitBinder> Comparer { get; set; }
        event Action<IUsageLimitBinder> ItemAdded;
        event Action<IUsageLimitBinder> ItemRemoved;
        void Invalidate();
        Task MountFromLimitLimitMasterDataAsync(CancellationToken cancellationToken = default);
        void SubscribeFromLimitLimitMasterData(Action? onChange = null, Action<Exception>? onError = null);
    }

    /// <summary>
    /// Lifecycle-free base for <see cref="UsageLimitBinderCollection"/>.
    /// Exposes the <see cref="IReadOnlyList{T}"/> contract over
    /// <see cref="IActionableUsageLimitBinder"/> and owns the
    ...
    /// </summary>
    public class ReadOnlyUsageLimitBinderCollection : IReadOnlyUsageLimitBinderCollection
    {
        ...
        // `Empty` instance.
        private readonly UsageLimitBinderCollection? _rootSource;
        private readonly Predicate<IReadOnlyUsageLimitBinder>? _predicate;
        private readonly List<IActionableUsageLimitBinder>? _cache;
        private bool _dead;

        // Shared sentinel list backing the `_dead` / `Empty` enumeration
        // paths. Reused so a dead view does not allocate per access.
        private static readonly List<IActionableUsageLimitBinder> EmptyList = new List<IActionableUsageLimitBinder>(0);

        /// <summary>
        /// Sentinel non-owning empty collection. Returned by <c>Where*</c>
        /// fast-exit paths (disposed view, dead view, default-id, no root).
        /// </summary>
        internal static ReadOnlyUsageLimitBinderCollection Empty { get; } = new ReadOnlyUsageLimitBinderCollection();
        ...
        /// </summary>
        protected ReadOnlyUsageLimitBinderCollection() { }

        /// <summary>
        /// Filter-view ctor. Registers the view with the owning root so
        /// subsequent source mutations refresh this view's cache.
        /// </summary>
        internal ReadOnlyUsageLimitBinderCollection(
            UsageLimitBinderCollection rootSource,
            Predicate<IReadOnlyUsageLimitBinder> predicate)
        {
            _rootSource = rootSource;
            _predicate = predicate;
            _cache = new List<IActionableUsageLimitBinder>();
        ...
        public virtual IActionableUsageLimitBinder this[int index]
            => (_dead || _cache == null) ? EmptyList[index] : _cache[index];

        /// <summary>Enumerator. Derived owning collection overrides.</summary>
        public virtual IEnumerator<IActionableUsageLimitBinder> GetEnumerator()
        ...
        /// </summary>
        protected virtual UsageLimitBinderCollection? GetRootSource() => _rootSource;
    ...
    /// <summary>
    /// Collection binder for UsageLimit.
    /// Wraps a list of UsageLimitBinder elements and reflects list-level
    ...
    /// </summary>
    public sealed class UsageLimitBinderCollection : ReadOnlyUsageLimitBinderCollection, IUsageLimitBinderCollection
    {
        private readonly Gs2Domain _gs2;
        private readonly IGameSession _session;

        // Owning binders are internal: consumers read the collection as the
        // non-owning IReadOnlyList<IActionableUsageLimitBinder> contract; only the
        // collection (and the wiring ItemAdded/ItemRemoved events) touch the
        // owning UsageLimitBinder so lifecycle (Subscribe/Dispose) never leaks.
        private readonly List<UsageLimitBinder> _binders = new List<UsageLimitBinder>();
        ...
        private readonly Dictionary<string, UsageLimitBinder> _bindersByRowKey = new Dictionary<string, UsageLimitBinder>();
        ...
        private readonly List<WeakReference<ReadOnlyUsageLimitBinderCollection>> _derivedViews = new List<WeakReference<ReadOnlyUsageLimitBinderCollection>>();
        ...
        private IComparer<IReadOnlyUsageLimitBinder> _configuredComparer = UsageLimitBinderComparer.Default;
        private IComparer<IReadOnlyUsageLimitBinder> _effectiveComparer =
            new UsageLimitBinderIdTieBreakComparer(UsageLimitBinderComparer.Default);
        ...
        /// OrderBy. Typed over the non-owning IReadOnlyUsageLimitBinder so
        /// it never exposes the owning binder.
        /// </summary>
        public IComparer<IReadOnlyUsageLimitBinder> Comparer
        {
            ...
            set
            {
                ...
                _effectiveComparer = new UsageLimitBinderIdTieBreakComparer(c);
                SortBinders();
                _onChange?.Invoke();
            }
        }
        ...
        /// IActionableUsageLimitBinder contract — the public surface never exposes
        /// the owning binder.
        /// </summary>
        public override IActionableUsageLimitBinder this[int index] => _binders[index];

        /// <summary>
        /// Enumerator over the owning binder list, yielding the non-owning
        /// IActionableUsageLimitBinder contract.
        /// </summary>
        public override IEnumerator<IActionableUsageLimitBinder> GetEnumerator() => _binders.GetEnumerator();
        ...
        /// </summary>
        protected override UsageLimitBinderCollection? GetRootSource() => this;
        ...
        // Wiring events carrying the owning binder contract (IUsageLimitBinder).
        // Public so a fake IUsageLimitBinderCollection can raise them; the ListHandler
        // subscribes to build/bind child handlers (which need the owning Subscribe),
        // then re-exposes its own public Action<IActionableUsageLimitBinder> events.
        /// <summary>Fires when a new element binder is added.</summary>
        public event Action<IUsageLimitBinder>? ItemAdded;

        /// <summary>Fires when an element binder is removed. The binder is
        /// Disposed after the event handlers return.</summary>
        public event Action<IUsageLimitBinder>? ItemRemoved;

        public UsageLimitBinderCollection(
            Gs2Domain gs2,
        ...
        /// </summary>
        internal void RegisterDerivedView(ReadOnlyUsageLimitBinderCollection view)
        {
            _derivedViews.Add(new WeakReference<ReadOnlyUsageLimitBinderCollection>(view));
        }
        ...
        /// <summary>One-shot Create + MountFromLimitLimitMasterDataAsync (no subscription).</summary>
        public static async Task<UsageLimitBinderCollection> CreateFromLimitLimitMasterDataAsync(
            Gs2Domain gs2,
            IGameSession session,
            CancellationToken cancellationToken = default)
        {
            var coll = new UsageLimitBinderCollection(gs2, session);
            await coll.MountFromLimitLimitMasterDataAsync(cancellationToken);
            return coll;
        }

        public async Task MountFromLimitLimitMasterDataAsync(CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
        ...
        public void SubscribeFromLimitLimitMasterData(Action? onChange = null, Action<Exception>? onError = null)
        {
            ThrowIfDisposed();
            ...
            _unsubscribers.Add(arrayLoader.Subscribe(
                ...
                async (_, _, items) =>
                {
                    ...
                    try
                    {
                        await ReconcileFromLimitLimitMasterItems(items, attachChildSubscribe: true, CancellationToken.None);
                    }
                    catch (Exception ex)
                    {
        ...
        private async Task ReconcileFromLimitLimitMasterItems(IList<EzLimitModel> items, bool attachChildSubscribe, CancellationToken cancellationToken)
        {
            ...
            {
                if (_disposed) return;
                var rowKey = ExtractLimitLimitMasterRowKey(item);
                if (rowKey == null) continue;
                seen.Add(rowKey);
                if (_bindersByRowKey.TryGetValue(rowKey, out var existing))
                {
                    ApplyLimitLimitMasterItemTo(existing.MutableModel, item);
                }
                else
                {
                    var binder = await BuildBinderFromLimitLimitMasterItem(item, cancellationToken);
                    if (_disposed)
                    {
                        binder.Dispose();
                        return;
                    ...
                    {
                        ApplyLimitLimitMasterItemTo(raced.MutableModel, item);
                        binder.Dispose();
                        continue;
                    }
        ...
        private async Task<UsageLimitBinder> BuildBinderFromLimitLimitMasterItem(EzLimitModel item, CancellationToken cancellationToken)
        {
            ...
            var binder = new UsageLimitBinder(model, _gs2, _session);
        ...
        internal sealed class UsageLimitBinderComparer : IComparer<IReadOnlyUsageLimitBinder>
        {
            public static readonly UsageLimitBinderComparer Default = new UsageLimitBinderComparer();
            public int Compare(IReadOnlyUsageLimitBinder? a, IReadOnlyUsageLimitBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
                if (b is null) return 1;
                return Comparer<UsageLimitId>.Default.Compare(a.Id, b.Id);
            }
        }

        internal sealed class UsageLimitBinderIdTieBreakComparer : IComparer<IReadOnlyUsageLimitBinder>
        {
            private readonly IComparer<IReadOnlyUsageLimitBinder> _inner;
            public UsageLimitBinderIdTieBreakComparer(IComparer<IReadOnlyUsageLimitBinder> inner) { _inner = inner; }
            public int Compare(IReadOnlyUsageLimitBinder? a, IReadOnlyUsageLimitBinder? b)
            {
                if (a is null && b is null) return 0;
                if (a is null) return -1;
