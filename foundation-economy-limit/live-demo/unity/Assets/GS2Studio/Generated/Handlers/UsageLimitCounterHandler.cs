...
{
    /// <summary>
    /// MonoBehaviour handler for UsageLimitCounter. Holds one UsageLimitCounterBinder,
    ...
    /// UsageLimitCounterListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimitCounter/UsageLimitCounter Handler")]
    public sealed class UsageLimitCounterHandler : UsageLimitCounterHandlerBase
    {
        ...
        private IUsageLimitCounterBinder? _binder;
        ...
        public override IActionableUsageLimitCounterBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="UsageLimitCounter"/>, so it is the model.</summary>
        public override UsageLimitCounter? Model => _binder;
        ...
        /// to substitute a fake UsageLimitCounterBinder; takes precedence over the
        ...
        /// </summary>
        public void SetKeys(string id, string limit)
        {
        ...
        /// <summary>
        /// Creates a UsageLimitCounterBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new UsageLimitCounterId(_id ?? string.Empty), new UsageLimitId(_limit ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned UsageLimitCounterBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a UsageLimitCounterBinderCollection) already
        ...
        /// </summary>
        public void Bind(IUsageLimitCounterBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IUsageLimitCounterBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IUsageLimitCounterBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IUsageLimitCounterBinder binder)
        {
