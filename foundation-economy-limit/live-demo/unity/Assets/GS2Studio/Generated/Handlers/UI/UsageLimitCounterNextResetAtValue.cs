...
{
    /// <summary>
    /// UI value bound to <c>UsageLimitCounter.nextResetAt</c>.
    /// Subscribes to the sibling <c>UsageLimitCounterHandler.Updated</c> event
    ...
    /// component alongside (or under) a <c>UsageLimitCounterHandler</c> or
    /// <c>UsageLimitCounterListItemHandler</c>; the handler is resolved
    ...
    /// persists it.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimitCounter/Value/NextResetAtValue")]
    public sealed class UsageLimitCounterNextResetAtValue : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(UsageLimitCounterNextResetAtValue)} on '{name}': no UsageLimitCounterHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke(model.NextResetAt);
        }
