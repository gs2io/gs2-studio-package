...
{
    /// <summary>
    /// UI button bound to <c>ResetEveryAllowance</c> on the sibling
    /// <c>UsageLimitCounterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>UsageLimitCounterHandlerBase.Binder.ResetEveryAllowance</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimitCounter/ButtonAction/ResetEveryAllowanceButton")]
    public sealed class UsageLimitCounterResetEveryAllowanceButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>ResetEveryAllowance</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(UsageLimitCounterResetEveryAllowanceButton)} on '{name}': no UsageLimitCounterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            try
            {
                await model.ResetEveryAllowance();
            }
            catch (Gs2Exception gs2Error)
            {
                UnityEngine.Debug.LogError($"UsageLimitCounterResetEveryAllowanceButton: ResetEveryAllowance failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"UsageLimitCounterResetEveryAllowanceButton: ResetEveryAllowance failed: {ex}");
                return;
            }
            finally
            {
