...
{
    /// <summary>
    /// UI button bound to <c>Reset</c> on the sibling
    /// <c>UsageLimitCounterHandlerBase</c>. Wires <c>UnityEngine.UI.Button.onClick</c>
    /// to <c>UsageLimitCounterHandlerBase.Binder.Reset</c> and forwards the
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimitCounter/ButtonAction/ResetButton")]
    public sealed class UsageLimitCounterResetButton : MonoBehaviour
    {
        ...
        /// in that window is dropped, not queued: a second <c>Reset</c>
        ...
        private async void OnClicked()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(UsageLimitCounterResetButton)} on '{name}': no UsageLimitCounterHandlerBase found in the parent chain; click ignored.", this);
                }
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError($"UsageLimitCounterResetButton: Reset failed: {gs2Error}");
            ...
            {
                UnityEngine.Debug.LogError($"UsageLimitCounterResetButton: Reset failed: {ex}");
                return;
            }
            finally
            {
