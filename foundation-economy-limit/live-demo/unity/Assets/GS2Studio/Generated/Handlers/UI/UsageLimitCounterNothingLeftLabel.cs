...
namespace GS2Studio.Generated.UsageLimitCounter.UI
{
    ...
    /// <c>{count} of {adBacked} uses are spent. Both presses are refused until the reset.</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimitCounter/TemplateLabel/NothingLeftLabel")]
    public sealed class UsageLimitCounterNothingLeftLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(UsageLimitCounterNothingLeftLabel)} on '{name}': no UsageLimitCounterHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.Count} of {model.AdMax} uses are spent. Both presses are refused until the reset.");
        }
    }
}
