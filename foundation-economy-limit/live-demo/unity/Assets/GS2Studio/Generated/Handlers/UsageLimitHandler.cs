...
{
    /// <summary>
    /// MonoBehaviour handler for UsageLimit. Holds one UsageLimitBinder,
    ...
    /// UsageLimitListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/UsageLimit/UsageLimit Handler")]
    public sealed class UsageLimitHandler : UsageLimitHandlerBase
    {
        ...
        private IUsageLimitBinder? _binder;
        ...
        public override IActionableUsageLimitBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="UsageLimit"/>, so it is the model.</summary>
        public override UsageLimit? Model => _binder;
        ...
        /// to substitute a fake UsageLimitBinder; takes precedence over the
        ...
        /// <summary>
        /// Creates a UsageLimitBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new UsageLimitId(_id ?? string.Empty), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned UsageLimitBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a UsageLimitBinderCollection) already
        ...
        /// </summary>
        public void Bind(IUsageLimitBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(IUsageLimitBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, IUsageLimitBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(IUsageLimitBinder binder)
        {
