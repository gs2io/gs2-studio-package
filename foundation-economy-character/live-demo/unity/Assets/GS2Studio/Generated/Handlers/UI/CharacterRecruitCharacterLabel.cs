...
namespace GS2Studio.Generated.CharacterRecruit.UI
{
    ...
    /// <c>Recruit {character}</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>CharacterRecruit</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterRecruit/TemplateLabel/CharacterLabel")]
    public sealed class CharacterRecruitCharacterLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(CharacterRecruitCharacterLabel)} on '{name}': no CharacterRecruitHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
