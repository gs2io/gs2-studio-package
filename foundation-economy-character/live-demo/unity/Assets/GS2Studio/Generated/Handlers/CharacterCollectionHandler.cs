...
{
    /// <summary>
    /// MonoBehaviour handler for CharacterCollection. Holds one CharacterCollectionBinder,
    ...
    /// CharacterCollectionListItemHandler, which reads its binder from the
    /// parent list instead of owning one.
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/CharacterCollection/CharacterCollection Handler")]
    public sealed class CharacterCollectionHandler : CharacterCollectionHandlerBase
    {
        ...
        // Construction seam. Defaults to DefaultCharacterCollectionBinderFactory.Instance;
        // override via SetBinderFactory to substitute a fake binder.
        private ICharacterCollectionBinderFactory? _binderFactory;

        private ICharacterCollectionBinder? _binder;
        ...
        public override IActionableCharacterCollectionBinder? Binder => _binder;

        /// <summary>The bound model, or null when no binder is attached. The
        /// binder implements <see cref="CharacterCollection"/>, so it is the model.</summary>
        public override CharacterCollection? Model => _binder;
        ...
        /// to substitute a fake CharacterCollectionBinder; takes precedence over the
        /// default factory. Pass null to revert to the default.
        /// </summary>
        public void SetBinderFactory(ICharacterCollectionBinderFactory? factory)
        {
            _binderFactory = factory;
        }

        private ICharacterCollectionBinderFactory ResolveBinderFactory()
            => _binderFactory ?? DefaultCharacterCollectionBinderFactory.Instance;
        ...
        /// <summary>
        /// Creates a CharacterCollectionBinder from the current identity keys, mounts it,
        ...
        public async Task ReloadAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var binder = await ResolveBinderFactory().CreateAsync(new CharacterCollectionId("charactercollection"), gs2, session, cancellationToken);
        ...
        /// <summary>
        /// Attaches an externally-owned CharacterCollectionBinder (lifecycle managed by the
        ...
        /// external subscription path (e.g. a CharacterCollectionBinderCollection) already
        ...
        /// </summary>
        public void Bind(ICharacterCollectionBinder binder, bool subscribe = true)
        {
            if (_isDestroyed) return;
            var generation = InvalidateBindingIntent();
            if (_binder == binder)
        ...
        private void Attach(ICharacterCollectionBinder binder, bool subscribe, int generation)
        {
            if (generation != _bindingIntentGeneration) return;
            _binder = binder;
        ...
        private bool IsCurrentBinding(int generation, ICharacterCollectionBinder binder)
            => !_isDestroyed
                && generation == _bindingIntentGeneration
                && ReferenceEquals(_binder, binder);

        private void SubscribeToBinder(ICharacterCollectionBinder binder)
        {
