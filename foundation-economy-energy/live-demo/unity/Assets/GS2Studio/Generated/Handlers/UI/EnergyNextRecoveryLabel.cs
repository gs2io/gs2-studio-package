...
namespace GS2Studio.Generated.Energy.UI
{
    ...
    /// <c>{nextRecoverdAt}</c>. Each <c>{key}</c> placeholder is
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Energy/TemplateLabel/NextRecoveryLabel")]
    public sealed class EnergyNextRecoveryLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EnergyNextRecoveryLabel)} on '{name}': no EnergyHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
