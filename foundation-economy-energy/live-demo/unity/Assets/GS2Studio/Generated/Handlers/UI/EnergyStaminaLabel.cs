...
namespace GS2Studio.Generated.Energy.UI
{
    ...
    /// <c>{currentValue}/{currentMaximumValue}</c>. Each <c>{key}</c> placeholder is
    /// substituted with the resolved value from <c>Energy</c>
    ...
    /// </summary>
    [AddComponentMenu("GS2 Studio/DomainType/Energy/TemplateLabel/StaminaLabel")]
    public sealed class EnergyStaminaLabel : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_handler == null)
            {
                ...
                if (!_warnedMissingHandler)
                {
                    ...
                    Debug.LogWarning(
                        $"{nameof(EnergyStaminaLabel)} on '{name}': no EnergyHandlerBase found in the parent chain; component inactive.", this);
                }
                return;
            }
            if (!_subscribed)
        ...
        {
            _onUpdate.Invoke($"{model.CurrentValue}/{model.CurrentMaximumValue}");
        }
    }
}
